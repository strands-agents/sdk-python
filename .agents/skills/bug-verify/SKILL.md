---
name: bug-verify
description: Local preview of the strands-agents/devtools `/strands bug-verify` agent. Body is the upstream Task Bug Verifier SOP verbatim — do not paraphrase. Use when the user types `/bug-verify`, asks to "verify a bug" or "triage an issue", references a bug-report GitHub issue and wants reproduction + urgency assessment, or wants to anticipate what the remote auto-bug-verify GitHub Action will do to an issue. The agent inspects the implicated source, runs a reproduction in a code executor, scores urgency, applies triage labels, and records a verification report (it does not comment on the issue). Findings are close but not identical to the remote agent. Strongly prefer running this skill in a fresh-context subagent rather than inline — the SOP is long and the verdict is more reliable when it isn't entangled with the parent conversation's prior context.
...
its own working reproduction (sharing that repro), and (2) when it couldn't
reproduce the bug at all and needs more information. Otherwise the verification
...
- post the single comment -> `gh issue comment <id> --body-file ...` (the two cases above only)
- When GITHUB_WRITE is not available locally, present the report, the label set,
  and any comment for the user to apply rather than writing to the issue.
-->
