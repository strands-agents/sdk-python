...
export interface RoutingStrategy {
   ...
   *
   * The router asks once before the first model call with an empty attempt history, then after each
   * unclaimed failure. Candidates are matched by identity; constructing an equivalent candidate here is
   * invalid.
   *
   * Declining the opening selection uses the router's default model. Declining after a failure ends
   * routing and preserves the pending model error. Throwing or returning an invalid candidate propagates
   * during opening selection; after a failed model call it ends routing without replacing that error.
   *
   * Each failure round may use a candidate once. Returning a candidate already used in the current round
   * ends routing. If a nested candidate cannot resolve to a model, that candidate consumes its round slot
   * and the strategy is asked again.
   *
