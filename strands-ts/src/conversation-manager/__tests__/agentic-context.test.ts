...
describe('summarizeContextTool', () => {
  ...
  it('assigns a durable id to the generated summary', async () => {
    // The summary message is spliced straight into agent.messages, bypassing the append
    // chokepoint, so summarizeContextTool must assign it a durable id itself.
    const model = mockModel('Summary')
    const messages = makeMessages(20)
    ...
    expect(typeof summaryMessage.id).toBe('string')
    expect(summaryMessage.id).toBeTruthy()
  })
