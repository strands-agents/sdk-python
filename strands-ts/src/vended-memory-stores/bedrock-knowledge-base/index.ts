...
export class BedrockKnowledgeBaseStore implements MemoryStore {
  ...
  async add(content: string, metadata?: Record<string, JSONValue>): Promise<BedrockKnowledgeBaseAddResult> {
    ...
    // Only `CUSTOM` and `S3` accept direct ingestion. The constructor already rejects a *writable*
    // store with any other type, so this guards the direct-misuse path (a non-writable store whose
    // `add` is called directly — `MemoryManager` never routes writes to one): fail fast with a clear
    // message instead of building a malformed document for Bedrock to reject with an opaque error.
    ...
    try {
      ...
      logger.error(
        `store=<${this.name}>, knowledgeBaseId=<${this._knowledgeBaseId}>, dataSourceId=<${dataSourceId}>, dataSourceType=<${this._dataSourceType}>, error=<${error}> | knowledge base document ingestion failed`,
        error
      )
      throw error
    }
