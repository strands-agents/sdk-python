...
abstract class BaseOffloadStrategy implements ContextStrategy {
  ...
  protected async _transformToolResultBlocks(message: Message, messages: Message[]): Promise<boolean> {
    ...
    for (let blockIndex = 0; blockIndex < message.content.length; blockIndex++) {
      ...
      const stashRef = await this._stashBlock(block)
      const replacement = await this._replaceToolResultBlock(block, tokens, stashRef)
  ...
  protected abstract _replaceToolResultBlock(
    block: ToolResultBlock,
    tokens: number,
    stashRef?: string
...
class OffloadDropStrategy extends BaseOffloadStrategy {
  ...
  protected async _replaceToolResultBlock(
    block: ToolResultBlock,
    _tokens: number,
    stashRef?: string
  ): Promise<ToolResultBlock | null> {
