/**
 * Summarize method: compresses the oldest messages into a summary,
...
import type { OffloadWhenConditions } from '../offload.js'
 ...
 *
 * Not instantiated directly — use the `Offload.summarize()` builder.
 *
 * @internal
 */
export class SummarizeMethod implements ContextStrategy {
  ...
  constructor(config?: SummarizeMethodConfig, conditions?: OffloadWhenConditions) {
    this._summaryRatio = Math.max(0.1, Math.min(0.8, config?.ratio ?? DEFAULT_SUMMARY_RATIO))
