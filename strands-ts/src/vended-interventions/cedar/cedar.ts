...
export class CedarAuthorization extends InterventionHandler {
  ...
  constructor(config: CedarAuthorizationConfig) {
    ...
    if (config.principalResolver) {
      ...
      this._principal = config.principal ?? {
        type: config.namespace ? `${config.namespace}::User` : 'User',
        id: 'anonymous',
      }
    }
  ...
  override beforeToolCall(event: BeforeToolCallEvent): InterventionAction {
    ...
    const result = isAuthorized({
      principal,
      action: { type: this._namespace ? `${this._namespace}::Action` : 'Action', id: event.toolUse.name },
      resource: { type: this._namespace ? `${this._namespace}::McpServer` : 'Resource', id: this._namespace ? 'default' : 'agent' },
      context: {
        input: toolInput,
        session: {
