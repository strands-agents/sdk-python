...
import { fullPromptTokens, isModelStreamEvent } from '../models/streaming.js'
...
export class Agent implements LocalAgent, InvokableAgent {
   ...
   *
   * Uses the token counting strategy: reads the full prompt the model processed (including cached
  ...
  private async _estimateInputTokens(streamOptions: StreamOptions): Promise<number> {
    ...
    if (lastAssistantIdx >= 0) {
      ...
      const knownBaseline = fullPromptTokens(usage) + usage.outputTokens
