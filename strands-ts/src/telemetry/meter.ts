...
import { accumulateUsage, createEmptyUsage, fullPromptTokens } from '../models/streaming.js'
...
export class Meter {
  ...
  /**
   * Projected context size for the next model call (full prompt including cached tokens plus output).
   */
  ...
  private _updateUsage(usage: Usage): void {
    ...
    this._latestContextSize = fullPromptTokens(usage)
    this._projectedContextSize = fullPromptTokens(usage) + usage.outputTokens
