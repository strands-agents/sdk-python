...
describe('htmlToMarkdown', () => {
  ...
  it.each(['\u200b', '\u00ad', '\ufeff', ' '])('drops javascript: href with invisible prefix %s', (prefix) => {
    const result = htmlToMarkdown(`<a href="${prefix}javascript:alert(1)">click</a>`)
    expect(result).not.toMatch(/javascript:/)
  ...
  it.each(['\u200b', '\u00ad', '\ufeff'])('drops javascript: img src with invisible prefix %s', (prefix) => {
    expect(htmlToMarkdown(`<img src="${prefix}javascript:alert(1)" alt="x">`)).not.toMatch(/javascript:/)
  })

  it('strips data-URI image blob with invisible prefix', () => {
    const blob = 'A'.repeat(100)
    const result = htmlToMarkdown(`<img src="\u200bdata:image/png;base64,${blob}" alt="alt">`)
