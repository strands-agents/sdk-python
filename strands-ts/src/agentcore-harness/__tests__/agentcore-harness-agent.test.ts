...
import { Message, ReasoningBlock, TextBlock } from '../../types/messages.js'
...
describe('AgentCoreHarnessAgent', () => {
  ...
  describe('stream', () => {
    ...
    it('returns a partial cancelled result when the request is aborted', async () => {
      const controller = new AbortController()
      const send = mockHarness(cancelledStream(controller))
      const agent = new AgentCoreHarnessAgent({ harnessArn: HARNESS_ARN, runtimeSessionId: SESSION_ID })
      ...
      expect(result).toMatchObject({
        ...
        lastMessage: { role: 'assistant', content: [new TextBlock('partial')] },
      })
