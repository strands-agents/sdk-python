...
import type { ContentBlock, ContentBlockData, MessageData, StopReason, TextBlock } from '../types/messages.js'
...
export class AgentCoreHarnessAgent implements InvokableAgent {
  ...
  private async *_stream(
    ...
    const stopReason: StopReason = cancelSignal?.aborted ? 'cancelled' : decoded.stopReason
    if (stopReason !== 'cancelled') throwIfUnrecoverable(decoded)
