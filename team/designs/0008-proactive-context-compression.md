...
- **`inputTokens`**: the number of *net new* input tokens reported by the model provider after a model call — those neither served from nor written to the prompt cache. Cache tokens are reported separately in `cacheReadInputTokens` / `cacheWriteInputTokens` but occupy the context window just the same, so the full prompt (all messages, system prompt, and tool definitions) is the sum of all three. It does not include output tokens generated by the model.
...
sequenceDiagram
    ...
    Note over CM: Read metadata.usage from last assistant message<br/>known = full prompt (inputTokens plus any cache read/write tokens) + outputTokens<br/>new = messages added since that assistant message<br/>estimate = known + estimateTokens(new)<br/>check: estimate / limit >= threshold?<br/>If yes: call reduceOnThreshold()

    Agent->>Model: stream(agent.messages)
    Model-->>Agent: response + usage { inputTokens, outputTokens, cache counters }
...
After the model call, the framework attaches `usage` (containing `inputTokens`, `outputTokens`, and any cache counters) to the assistant message as metadata ([#2125](https://github.com/strands-agents/harness-sdk/pull/2125)). This metadata is part of the message itself, so it survives session persistence and restore. At the start of the next cycle, the base class reads it directly from `agent.messages`:

- **Known**: the full prompt (`inputTokens` plus the cache counters) plus `outputTokens`, from the last assistant message's metadata. This is the exact token count for everything the model saw, plus everything it generated (which is now part of the messages).
- **Unknown**: any messages appended after that assistant message, primarily tool results. These need to be estimated.

### Token Counting Strategy

The base class reads the full prompt (`inputTokens` plus the cache counters) and `outputTokens` from the last assistant message's metadata, then estimates any new messages added after it:
...
if usage is available:
    knownBaseline = contextTokenCount(usage)  # full prompt, cache tokens included, + outputTokens
