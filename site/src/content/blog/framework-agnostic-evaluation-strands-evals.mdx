...
If you are comparing agents built from different frameworks, you need a way to evaluate them
under identical criteria. Running Claude Agents through one evaluation tool and Google ADK
through another gives you a rough sense of how they perform, but not an exact comparison. One
tool might count a partial answer as success; another might fail it. Without a shared
evaluation setup, you are left guessing whether performance differences are attributed to the agents
themselves or from evaluation discrepancies.

Today, [Strands Evals](https://github.com/strands-agents/evals) is adding support for traces
from several new agent frameworks, including those following GenAI semantic conventions. This
is part of a larger initiative to make Strands Evals framework-agnostic, simplifying
cross-framework comparison. The table below shows the updated agent framework and tracing
combinations Strands Evals supports. Built-in refers to the framework's native OTel tracing.
OpenInference and Traceloop are third-party instrumentor libraries that hook into agent
frameworks to produce OTel spans.
...
- Why framework-specific evaluation tools fall short when you need to compare agents across
  frameworks.
- How Strands Evals converts raw OTel spans from any supported framework into a unified
  evaluation session.
- How to evaluate a Claude Agents SDK agent and an OpenAI Agents SDK agent using the same
  experiment setup.
- How to retrieve and evaluate production traces from a Google ADK agent deployed to Amazon
  Bedrock AgentCore via CloudWatch.
- Which observability backends Strands Evals supports as trace providers.

## Where framework-specific evaluation falls short

Many agent frameworks ship with native evaluation tools.

- LangChain and [LangSmith](https://www.langchain.com/langsmith)
- Google ADK and [Gen AI Evaluation Service](https://cloud.google.com/vertex-ai/generative-ai/docs/models/evaluation-overview)
- Microsoft AutoGen and [Azure AI Evaluation SDK](https://learn.microsoft.com/en-us/azure/ai-foundry/how-to/develop/evaluate-sdk)
- Pydantic AI and [Logfire](https://logfire.pydantic.dev/)

They offer feature-rich evaluation tailored to their native framework. While framework-specific
tools handle their own frameworks well, they face technical challenges when evaluating agents
from other frameworks. Agent frameworks often differ in instrumentation (the hooks emitting
information about what the agent did) and in the subsequent span format. Evaluating an agent
from one framework in another vendor's tool requires span format compatibility, or manual
conversion. This conversion can range from a small adapter to a significant engineering effort.
...
A framework-agnostic evaluation eliminates both issues. You define your cases and evaluators
once. When you try a new agent framework, you swap the agent code and keep everything else the
same. Comparisons become apples-to-apples: same cases, same evaluators, same scoring. This
ensures differences in results come from the agents, not the evaluation setup. Switching costs
also drop because you learn one evaluation tool and bring it with you as you change frameworks.

## How Strands Evals bridges frameworks

![Figure 1: The user-defined task converts raw traces into sessions using mappers and assembles
the task output](./resources/framework-agnostic-evaluation.png)

_Figure 1: `TaskOutput` composition. The session (trajectory) is produced by a session mapper
from raw OTel spans. The output, interactions, and environment are user-defined and vary by
experiment setup._
...
the agent did) in a standardized format and comprise a major part of the `TaskOutput`. The rest
of the evaluation pipeline operates on the `TaskOutput` the same way it would for a native
Strands agent. The other parts of `TaskOutput` (the agent output, interactions, and
environment) vary by experiment setup and are therefore user-defined.
...
The following sections walk through how to evaluate a commit message agent built with Claude
Agents SDK using Strands Evals, from initial setup to the final report. This walkthrough
assumes familiarity with Strands Evals. If you are new, see the
...
```bash
pip install strands-agents-evals claude-agent-sdk openinference-instrumentation-claude-agent-sdk opentelemetry-sdk
```
...
from strands_evals.otel import detect_otel_mapper, readable_spans_to_dicts
...
The following example swaps in OpenAI Agents to evaluate the same commit message task,
demonstrating what changes and what stays the same.
...
production, agents are more likely to run on managed infrastructure, where traces flow through
observability pipelines. Amazon Bedrock AgentCore supports various agent frameworks and exports
their OTel traces to Amazon CloudWatch via an ADOT (AWS Distro for OpenTelemetry) collector.
Despite these differences, Strands Evals can evaluate CloudWatch spans the same way it
evaluates in-process ones.

To demonstrate that a non-AWS agent framework deployed on AWS infrastructure still produces
evaluable traces, this section deploys a Google ADK commit message agent to AgentCore,
retrieves its production traces from CloudWatch, and runs them through the same Strands Evals
experiment used in the earlier examples. The evaluation code stays the same; only the trace
source changes.
...
**Amazon OpenSearch** is for agents exporting OTel spans to an OpenSearch cluster (self-hosted
or Amazon OpenSearch Service). It queries the `otel-v1-apm-span-*` index pattern by session ID
and maps the results through a dedicated session mapper.
...
# Amazon OpenSearch Service with SigV4
...
differences in results reflect differences in the agents, not the evaluation tooling. Whether
you are choosing your first agent framework or comparing several in production,
framework-agnostic evaluation gives you a stable measurement baseline that evolves with the
ecosystem rather than locking you into one corner of it.

The same principle extends to where your traces live. Whether you collect spans in-process
during development or retrieve them from production backends, the evaluation experience is
identical. The framework, the instrumentor, and the trace source all vary; the scoring does not.
...
- See [Remote Trace Providers](/docs/user-guide/evals-sdk/how-to/trace_providers/) for full
  configuration options for CloudWatch, Langfuse, and OpenSearch.
- Share your [feedback and feature requests](https://github.com/strands-agents/evals/issues) on
  GitHub.
