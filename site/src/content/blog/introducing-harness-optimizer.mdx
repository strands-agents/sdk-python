---
title: "Introducing Harness Optimizer: An Automatic Solution for the Agent Harness"
date: 2026-07-06
...
![Manual prompt iteration vs. automatic harness optimization](./resources/harness-optimizer/manual-vs-automatic.png)
...
![FormulaOptimizer architecture](./resources/harness-optimizer/formula-optimizer-architecture.png)
...
![Formula optimization loop](./resources/harness-optimizer/formula-optimization-loop.png)
...
![Contrastive reflection](./resources/harness-optimizer/contrastive-reflection.png)
...
![Multi-agent optimizer](./resources/harness-optimizer/multi-agent-optimizer.png)

## A full loop, end to end

We can wire the Formula and Optimizer up given a rollout engine (in the code snippet, we use the [`LocalRolloutEngine`](https://github.com/strands-labs/harness-optimizer/blob/main/strands_harness_optimizer/rollout_engines/local_engine.py) running on the local server), which generates the agent rollouts with the Formula to run the optimization with `Optimizer.fit()` as:

```python
from strands_harness_optimizer import SystemPromptFormula, LocalRolloutEngine, Trainer
from strands import Agent
from strands_harness_optimizer.formulas import SystemPromptFormula
from strands_harness_optimizer.adapters import apply_formulas_on_strands_agent
...
The HarnessOptimizer improves the model performance on AppWorld by **+23 points** (72.6 → 95.8).
...
- You have a reward signal to separate good from bad, whether a programmatic check or an LLM judge, e.g., [AgentCore Eval](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/optimization.html) and [Strands Eval](https://github.com/strands-agents/harness-sdk/tree/main/strands-py/strands-evals).
...
- Docs: the user guide walks through [Formulas](https://github.com/strands-labs/harness-optimizer#formulas), [Adapters](https://github.com/strands-labs/harness-optimizer#adapters), [Rewards](https://github.com/strands-labs/harness-optimizer#rewards), [Optimizers](https://github.com/strands-labs/harness-optimizer#optimizers), and the [training loop](https://github.com/strands-labs/harness-optimizer#training-loop).
