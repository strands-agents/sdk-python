...
```bash
pip install "strands-vllm[drift]"
```
...
async def main():
    ...
    await agent.invoke_async("Do I need a jacket in Seattle today?")
...
- Unknown tool: `Error: unknown tool: fake_tool | allowed_tools=[http_request, ...]`
- Missing argument: `Error: tool_name=<http_request> | missing required argument(s): url`
- Unknown argument: `Error: tool_name=<http_request> | unknown argument(s): invalid_param`
