...
Key options for both the store and each `namespaces` entry:
...
- **One writable store per session.** At most one store may be `writable` for a given
  `(actorId, sessionId)`. Constructing more than one throws.
- **Placeholders resolve at write time.** Only `{actorId}` and `{sessionId}` are substituted
  client-side. Any other placeholder in a namespace causes construction to throw. Provision
  strategies with `{actorId}`/`{sessionId}`-only templates to match.
