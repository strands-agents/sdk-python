...
| `languages` | ✅ | At least one of `python` / `typescript`, each with `package` + `registry` (a `https://pypi.org/` URL for Python, `https://www.npmjs.com/` for TypeScript). For a vendor-guide integration with no Strands-specific package, leave the block empty (`python: {}`) and set `docsUrl` to your guide. |
