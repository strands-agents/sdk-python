...
# Tool execution
DEBUG | strands.tools.executors._executor | tool_use=<{'toolUseId': 'tooluse_kZ3N...', 'name': 'http_request', ...}> | streaming
```

With `load_tools_from_directory=True`, the registry also scans a local `./tools/`
directory, validates what it finds, and hot-reloads changes:
...
DEBUG | strands.tools.registry | tool_modules=<['weather']> | discovered
...
DEBUG | strands.tools.registry | tool_name=<weather> | loaded dynamic tool config

# Tool hot reloading
DEBUG | strands.tools.registry | tool_name=<weather> | searching directories for tool
DEBUG | strands.tools.registry | tool_name=<weather> | reloading tool
DEBUG | strands.tools.registry | tool_name=<weather> | successfully reloaded tool
```
