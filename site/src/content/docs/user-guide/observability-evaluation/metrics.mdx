...
When the model provider reports pricing data, `result.cost` contains the estimated
cost in USD for that invocation. It includes all model calls made during the
invocation and remains attached to that result if you invoke the same agent again.
The value is `None` when the provider does not report cost.
