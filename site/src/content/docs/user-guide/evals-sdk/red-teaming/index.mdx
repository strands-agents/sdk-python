...
Generate adversarial cases from the agent, run a strategy against it, and read the breaches:
...
report = RedTeamExperiment(
    cases=cases, agent=agent, attack_strategies=[CrescendoStrategy()]
).run_evaluations()
report.display()
```

The [Quickstart](quickstart.md) walks through each step and shows the report output.
...
A strategy runs against the agent one case at a time (`max_workers=1` is enforced — strategies share the target and per-case state). Each strategy carries its own cheap in-loop "should I stop?" gate, but the authoritative breach verdict always comes from the `AttackSuccessEvaluator` over the full conversation and tool trace.

## Best Practices

These follow established LLM red-teaming guidance (the [OWASP Top 10 for LLM Applications](https://owasp.org/www-project-top-10-for-large-language-model-applications/) and the [NIST AI Risk Management Framework](https://www.nist.gov/itl/ai-risk-management-framework)), scoped to what this module does.
