...
## Strategy gate vs. authoritative verdict

Each strategy carries its own internal judge to decide when to stop attacking early — that's the strategy's `success_threshold`. It is a cheap optimization and uses a strategy-specific rubric. The number you report and act on is the `AttackSuccessEvaluator` score, computed once over the whole conversation after the attack finishes. The two can disagree (a strategy may stop early on its own gate while the authoritative judge scores the full trace differently), and the evaluator is the one that decides a breach.

## Throttling and retries

Red teaming makes many sequential model calls — the target, the attacker (for LLM-driven strategies), and the judge, across every turn of every case. Transient throttling from the model provider is retried automatically: the Strands Agents SDK retries throttled model calls by default, and the evals experiment layer retries the attack task and the evaluator on throttling errors. You don't need to add retry logic around `run_evaluations()`. For large runs, configure proactive rate limiting on your model provider (for Bedrock, an adaptive-retry `boto_client_config`).
