...
cases = [
    Case(
        ...
        metadata={"task_description": "Current temperature reported from the API"},
    ),
]
...
experiment = Experiment(
    ...
    evaluators=[
        OutputEvaluator(
            rubric="Score 1.0 if a temperature is reported. Score 0.0 otherwise."
        ),
...
async def main():
    report = await experiment.run_evaluations_async(weather_agent, max_workers=1)
