...
**[ToolSelectionAccuracyEvaluator](tool_selection_evaluator.md)**

- **Level**: TRACE_LEVEL
- **Purpose**: Evaluate whether correct tools were selected
- **Use Case**: Assess tool choice accuracy in multi-tool scenarios

**[ToolParameterAccuracyEvaluator](tool_parameter_evaluator.md)**

- **Level**: TRACE_LEVEL
- **Purpose**: Evaluate accuracy of tool parameters
- **Use Case**: Verify correct parameter values for tool calls

### Skill Usage Evaluators

These evaluate agents equipped with skills: reusable `SKILL.md` instruction files loaded
at runtime. Routing and execution fail for different reasons, so they are scored
separately.

**[SkillSelectionAccuracyEvaluator](skill_selection_evaluator.md)**

- **Level**: Per invoked skill
- **Purpose**: Evaluate whether loading each skill was appropriate for the task
- **Use Case**: Detect skills loaded for tasks they don't cover, and diagnose
  overlapping skill descriptions

**[SkillInstructionFollowingEvaluator](skill_instruction_following_evaluator.md)**

- **Level**: Per invoked skill
- **Purpose**: Measure how fully the agent followed an invoked skill's prescribed steps
- **Use Case**: Find the step an agent skipped behind a fluent-looking response
...
- **Includes**: `Equals`, `Contains`, `StartsWith`, `ToolCalled`, `SkillInvoked`, `StateEquals`
