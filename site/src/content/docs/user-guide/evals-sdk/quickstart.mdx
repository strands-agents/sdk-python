...
test_cases = [
    Case[str, str](
        ...
        metadata={"category": "weather", "expected_tools": ["http_request"]}
    ),
    Case[str, str](
        name="weather-2",
    ...
    Case[str, str](
        ...
        "&current=temperature_2m",
        expected_trajectory=["http_request", "http_request"],
        metadata={"category": "multi_tool", "expected_tools": ["http_request"]}
    )
]
...
sample_agent = Agent(tools=[http_request])
...
def user_task_function():
    return Agent(tools=[http_request], callback_handler=None)
...
tool_context = """
Available tools:
- http_request(method: str, url: str) -> dict: Fetch a URL and return the response
...
async def generate_experiment():
    ...
    experiment = await generator.from_context_async(
        ...
        num_cases=5,
        evaluator=TrajectoryEvaluator,
        task_description="Research and file assistant",
