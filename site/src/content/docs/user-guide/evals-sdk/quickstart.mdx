...
def get_response_with_tools(case: Case) -> dict:
    agent = Agent(
        tools=[calculator],  # calculator is defined at the end of this page
...
test_cases = [
    ...
    Case[str, str](
        name="complex-1",
        input="What is 25 * 48, and then divide that by 4?",
        expected_trajectory=["calculator", "calculator"],
        metadata={"category": "multi_tool", "expected_tools": ["calculator"]}
    )
]
...
`--agent MODULE:ATTR` points at the same factory you'd pass to `experiment.run_evaluations` in Python. See the [CLI reference](cli.md) for `report`, `diagnose`, `generate`, CI exit codes, and the full flag set.
...
tool_context = """
Available tools:
- calculator(a: float, b: float, op: str) -> float: Apply an arithmetic operator to two numbers
- file_read(path: str) -> str: Read file contents
- file_write(path: str, content: str) -> str: Write content to a file
"""
...
async def generate_experiment():
    ...
    experiment = await generator.from_context_async(
        ...
        num_cases=5,
        evaluator=TrajectoryEvaluator,
        task_description="Assistant with calculation and file tools",
...
- [Command-Line Interface](cli.md) - Run experiments and diagnose sessions from the shell or CI
