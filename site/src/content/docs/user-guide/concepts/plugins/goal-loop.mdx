...
1. `BeforeInvocationEvent` starts a fresh run (or continues an in-progress one).
2. The agent processes the prompt and produces a response.
3. `AfterInvocationEvent` fires. GoalLoop extracts the last assistant message and runs the validator.
4. If the validator passes, the run terminates with `stop_reason="satisfied"`.
5. If the validator fails and budget remains, GoalLoop sets `event.resume` with feedback. The agent re-invokes with that feedback as a new user message.
6. If `max_attempts` or `timeout` is exhausted, the run terminates without retrying.
...
When both `max_attempts` and `timeout` are left unbounded (the defaults), the plugin warns at construction time. Set at least one bound in production to prevent runaway loops.

## Advanced Usage

### Inspecting Results

After an invocation completes, call `last_result()` / `lastResult()` with the agent to get the full attempt history:
