...
This XML block is refreshed before each invocation, so changes to available skills (through <Syntax py="set_available_skills" ts="setAvailableSkills" />) take effect immediately. Activated skills are tracked in [agent state](../agents/state.mdx) for session persistence.
...
### Keeping instructions resident with `injection_mode`

:::note[Python only]
The `injection_mode` option is currently available in the Python SDK. TypeScript support is planned as a follow-up.
:::

The default progressive-disclosure flow delivers a skill's instructions once, as a tool result. In long-running, multi-turn workflows those instructions can drift out of the model's attention as the conversation grows. For those cases, pass `injection_mode="system_prompt"` to keep activated skills' full instructions resident in the per-call system prompt instead:

```python
from strands import Agent, AgentSkills

agent = Agent(plugins=[
    AgentSkills(skills=["./skills/"], injection_mode="system_prompt"),
])
```

Both modes expose exactly one tool named `skills` — only its signature differs:

- **`"tool"` (default)** — `skills(skill_name)` activates a skill and returns its full instructions as the tool result. Metadata is injected durably into `agent.system_prompt`.
- **`"system_prompt"`** — `skills(action, skill_name)` where `action` is `"activate"` or `"deactivate"`. Activating a skill puts its full instructions (and `allowed-tools` guidance) into the system prompt, where they persist across turns; deactivating removes them to reclaim context.

In `system_prompt` mode, delivery is **ephemeral**: the skills block is re-rendered from the current activation state and appended to the per-call system prompt through an internal [Context Injector](./context-injector) — `agent.system_prompt` itself is never modified. Activation state lives in [agent state](../agents/state.mdx), so it persists across runs through the session manager. Programmatic equivalents (`activate_skill_for` / `deactivate_skill_for`) are available on the plugin.

Note that resident instructions consume context window on every call — prefer the default `"tool"` mode when many skills are available or instructions are large, and `"system_prompt"` mode when a small number of skills must stay salient across a long workflow.
...
Skill names must match the parent directory name. By default, validation issues produce warnings rather than errors. Pass <Syntax py="strict=True" ts="strict: true" /> to raise exceptions instead.
:::
...
| `injection_mode` | `SkillInjectionMode` | `"tool"` | Keyword-only. `"tool"` returns activated instructions as a tool result; `"system_prompt"` keeps activated skills' instructions resident in the per-call system prompt with `activate`/`deactivate` toggles. |
