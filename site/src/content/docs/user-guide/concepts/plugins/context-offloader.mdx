...
All backends implement the `Storage` protocol and preserve
content type metadata, so you can also build your own.

:::note[Storage keys]
The Context Offloader's storage backends use the unified
[Storage](../storage.mdx) interface. Keys are opaque strings —
the shipped backends use `/`-separated path-like keys, but
custom backends may use any key format they choose.
:::
