...
| `BeforeToolsEvent`                 | Triggered once before a batch of tools is executed, before any `BeforeToolCallEvent`                          |
...
| `AfterToolsEvent`                  | Triggered once after a batch of tools finishes. Always fires (even on cancel). Uses reverse callback ordering |
