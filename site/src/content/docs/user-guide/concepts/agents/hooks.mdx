...
- [`AfterToolsEvent`](@api/python/strands.hooks.events#AfterToolsEvent)
    - `end_turn` - End the agent loop after the current tool batch without another model call. Set it to `True` for the default assistant message, or to a non-empty string to use that string as the final assistant message. The result uses `stop_reason="end_turn"`.
