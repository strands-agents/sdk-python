...
class ResultProcessor(HookProvider):
    ...
    def process_result(self, event: AfterToolCallEvent) -> None:
        if event.tool_use.name == "http_request":
            # Truncate long response bodies before they reach the model
            original_content = event.result["content"][0]["text"]
            event.result["content"][0]["text"] = original_content[:4000]
```
</Tab>
<Tab label="TypeScript">
...
class ResultProcessor(HookProvider):
    ...
    def process_result(self, event: AfterToolCallEvent) -> None:
        if event.tool_use.name == "http_request":
            original_content = event.result["content"][0]["text"]
            logger.info(f"Truncating http_request result: {len(original_content)} chars")
            event.result["content"][0]["text"] = original_content[:4000]
```
</Tab>
<Tab label="TypeScript">
...
For example, to force every request the `http_request` tool makes to be a read-only
GET, whatever method the model asks for:
...
agent = Agent(tools=[http_request], hooks=[fix_parameters])
result = agent("Fetch https://api.github.com/zen")
```
</Tab>
<Tab label="TypeScript">
...
For example, to limit the `http_request` tool to 3 calls per invocation:
...
limit_hook = LimitToolCounts(max_tool_counts={"http_request": 3})

agent = Agent(tools=[http_request], hooks=[limit_hook])

# Only the first three requests succeed
agent("Check the current weather in Seattle, Tokyo, Miami, and London")
# The count resets every invocation, so this call can make requests again
agent("Now check Berlin")
```
</Tab>
<Tab label="TypeScript">
...
```typescript
--8<-- "user-guide/concepts/agents/hooks.ts:model_call_retry_class"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/concepts/agents/hooks.ts:model_call_retry_usage"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/concepts/agents/hooks.ts:tool_call_retry_class"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/concepts/agents/hooks.ts:tool_call_retry_usage"
```
</Tab>
</Tabs>
