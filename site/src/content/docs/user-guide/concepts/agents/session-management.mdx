...
same session ID and agent ID. A manager seeds its message index from the last message it
saw when it restored the session, then increments that index locally. Managers whose
invocations overlap claim the same index, and the second write replaces the first agent's
turn. Invocations that do not overlap merge instead: the later agent
...
the same instance to every agent. Then build an agent per request, invoke it, and let it
go out of scope. An agent binds to its session at construction time, so a long-lived
agent shared across conversations cannot switch between them.
