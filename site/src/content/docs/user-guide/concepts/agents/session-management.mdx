...
Only Python catches part of this: handing one session manager to two agents with the same agent ID raises `SessionException`. Nothing catches two agents that each hold their own session manager, whether that is a second Lambda execution or a server that builds a manager per request. Their invocations either overlap, and one agent's turn is overwritten, or they do not, and the second agent restores the first one's messages and appends to them, leaving one history that holds two conversations.

A single agent instance does reject overlapping invocations, described in [Concurrent Invocations](./agent-loop.md#concurrent-invocations), but that guard is in-process: it sees nothing running anywhere else.

### Create an Agent per Conversation

Constructing an agent is cheap, since it wires up tools, hooks, and plugins in memory and makes no model call. Build one per request, invoke it, and let it go out of scope. An agent binds to its session at construction, so a long-lived agent shared across conversations cannot switch between them. The model provider is the part worth reusing: providers such as `BedrockModel` build their client in the constructor, so create the provider once per process and pass the same instance to every agent.
