...
Strands recommends `SnapshotSessionManager` for new agents. It pairs with a [unified `Storage`](@api/python/strands.storage.storage#Storage) backend that decides where snapshots live:

1. **`LocalFileStorage`**: Stores snapshots on the local filesystem
2. **`S3Storage`**: Stores snapshots in Amazon S3

The same `Storage` backend also powers other SDK subsystems, so you configure a storage location once and reuse it.

The message-log managers ([`FileSessionManager`](@api/python/strands.session.file_session_manager#FileSessionManager), [`S3SessionManager`](@api/python/strands.session.s3_session_manager#S3SessionManager)) remain available but are deprecated. They persist each message as a separate object and emit a `DeprecationWarning`.
...
`LocalFileStorage` writes snapshots to the local filesystem. Pass it to `SnapshotSessionManager`:
...
session_manager = SnapshotSessionManager(
    "user-123",
    storage=LocalFileStorage("/path/to/sessions"),  # Optional, defaults to a temp directory
)

agent = Agent(session_manager=session_manager)

# State and messages are persisted automatically on every turn
agent("Hello, I'm a new user!")
```

`SnapshotSessionManager` covers both single agents and multi-agent orchestrators (see [Multi-Agent Sessions](#multi-agent-sessions)).
...
<Tab label="Python">
  ...
  `SnapshotSessionManager` with `LocalFileStorage` writes one snapshot blob per save, under this layout:
            ...
            └── agent/
                └── <agent_id>/
                    └── snapshots/
                        ...
                        └── immutable_history/
                            └── snapshot_<id>.json    # Immutable checkpoint
```

The key layout matches the TypeScript SDK. The snapshot contents differ per SDK, so a session is restored by the SDK that wrote it.
</Tab>
...
For cloud-based persistence, especially in distributed environments, use `S3Storage`:
...
# State and messages are persisted to S3 on every turn
...
Snapshot ids carry a timestamp prefix, so they sort in chronological order. List them, then restore a specific checkpoint by id:

<Tabs>
<Tab label="Python">

```python
# list_snapshot_ids and restore_snapshot are async
