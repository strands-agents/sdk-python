...
- **Two callers creating the same session at the same time.** Session creation is a check followed by a write, not an atomic operation, so simultaneous cold starts on a new session ID either both succeed, with the later write winning, or the loser raises `SessionException`.
