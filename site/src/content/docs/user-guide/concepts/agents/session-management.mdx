...
A session manager assumes it is the only writer for its key: session ID plus agent ID, or
session ID plus orchestrator ID for a Graph or Swarm. Lost or interleaved conversation
history almost always traces back to that assumption being broken.

### One Conversation per Session

Give each conversation its own session ID. The session ID and the agent ID
(<Syntax py="agent_id" ts="id" />) together address one conversation thread in storage,
so a second conversation with the same agent needs a second session ID.

Several agents can share one session ID as long as their agent IDs differ. The session
acts as a namespace, and each agent keeps its own message history and state inside it.
Agents inside a Graph or Swarm are the exception: only the orchestrator takes a session
manager, as described under [Multi-Agent Sessions](#multi-agent-sessions). What corrupts
data is two live agents addressing the same session ID and agent ID.

<Tabs>
<Tab label="Python">

Handing one session manager to two agents with the same agent ID fails immediately with
a `SessionException`: agent IDs must be unique within a session. The default `agent_id`
is `"default"`, so two agents that both leave it unset collide.

Nothing detects two agents that each have their own session manager, whether they run in
separate processes or in one process that builds a manager per request, addressing the
same session ID and agent ID. A manager seeds its message index from the
last message it saw when it restored the session, then increments that index locally.
Managers whose invocations overlap claim the same index, and the second write replaces
the first agent's turn. Invocations that do not overlap merge instead: the later agent
restores the earlier one's messages and appends to them, leaving one history that holds
two conversations.

A brand-new session can also fail outright: the session manager creates the session
record when you construct it, and a record that another caller just created raises
`SessionException`. That check is not atomic, so two simultaneous cold starts can instead
both succeed, with the later write winning. Treat the exception as a symptom of duplicate
callers, not a guard against them.

</Tab>
<Tab label="TypeScript">

Nothing detects two agents addressing the same session ID and agent ID. The default
agent `id` is `'agent'`, so two agents that both leave it unset collide.

Every save overwrites `snapshot_latest` with the saving agent's full in-memory history.
When invocations overlap, the last writer wins and the other agent's turn disappears even
though both reported success. When they do not overlap, the later agent restores the
earlier one's history and continues it, leaving one history that holds two conversations.

</Tab>
</Tabs>

A single agent instance rejects overlapping invocations, described in
[Concurrent Invocations](./agent-loop.md#concurrent-invocations). That guard is
in-process, so it sees nothing happening outside the current process.

### Create an Agent per Conversation

Constructing an agent is cheap: it wires up tools, hooks, and plugins in memory and makes
no model call. The model provider often costs more, since providers such as `BedrockModel`
build their client in the constructor, so create the provider once per process and hand
the same instance to every agent. Then build an agent per request, invoke it, and
let it go out of scope. An agent binds to its session at construction time, so a
long-lived agent shared across conversations cannot switch between them.

<Tabs>
<Tab label="Python">

Creating an agent with a session manager also restores the session. `FileSessionManager`
and `S3SessionManager` read one object per stored message, so a long conversation costs
more to restore than a short one.

The session manager is per request as well, since it binds to a single session ID. Its
constructor builds an S3 client, so in a single-threaded environment such as AWS Lambda,
pass a `boto_session` created once per process to avoid repeating that work. The example
below leaves it out, since one `boto3.Session` cannot be shared safely across threads.
...
def handle_request(conversation_id: str, prompt: str) -> str:
...
Restore runs on the first invocation rather than in the constructor, so constructing the
agent does no I/O. The storage backend holds no session state, so build it once as well
and give each request its own session manager.

```typescript
--8<-- "user-guide/concepts/agents/session-manager_imports.ts:per_request_agent_imports"

--8<-- "user-guide/concepts/agents/session-manager.ts:per_request_agent"
```

</Tab>
</Tabs>

### Serializing Access to a Shared Session

When two invocations for the same session ID can overlap, the lock has to live outside
Strands. The built-in session managers take no lock, and the per-instance invocation
guard cannot see an agent running in another process.

AWS Lambda is where this surfaces most often: a retried or duplicated event starts a
second execution with the same session ID, that execution builds its own agent, and both
write the same keys. Queue redelivery, a double-submitted form, and a user with two open
browser tabs produce the same race. Deduplication that lives inside one agent instance,
including the Python SDK's idempotency token, does not help, because each retry arrives
in a fresh process with a fresh agent.

Two approaches work:

- **A distributed lock keyed by session ID.** Take it before constructing the agent,
  release it after the invocation completes, and give it a TTL or lease so a crashed
  holder cannot block the session forever. A conditional write on a DynamoDB item and
  `SET key value NX PX` in Redis both do the job. Reject the duplicate request rather
  than waiting on the lock when the caller can retry.
- **A per-session FIFO queue.** Route requests through a FIFO queue partitioned by
  session ID (in Amazon SQS, the session ID becomes the message group ID) so the second
  request queues behind the first instead of racing it.

Use the lock when a caller is waiting on the response, and the queue when the work can
...
- **Concurrent Access**: Session managers are not thread-safe and take no distributed
  lock. Keep one writer per session, and serialize overlapping invocations yourself: see
  [Sessions, Agents, and Concurrency](#sessions-agents-and-concurrency).
