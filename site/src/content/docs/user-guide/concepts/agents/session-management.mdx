...
Session storage is keyed by session ID and agent ID, or by session ID and orchestrator ID
for a Graph or Swarm, and a session manager assumes it is the only writer for that key.
Lost or interleaved conversation history almost always traces back to that assumption
being broken.
...
so a second conversation needs a second session ID.
...
Two agents in separate processes, each with its own session manager pointed at the same
session ID and agent ID, is not detected. A manager seeds its message index from the last
message it saw when it restored the session, then increments that index locally. Managers
whose invocations overlap claim the same index, and the second write replaces the first
agent's turn. Invocations that do not overlap merge instead: the later agent restores the
earlier one's messages and appends to them, leaving one history that holds two
conversations.

A brand-new session can also fail outright. The session manager creates the session
record when you construct it, and creating a record that another process just created
...
A single agent instance already rejects overlapping invocations by default, described
in [Concurrent Invocations](./agent-loop.md#concurrent-invocations). That guard is
...
no model call. The model provider costs more, because its constructor builds the client
that talks to the service, so create the provider once per process and hand the same
instance to every agent. Then build an agent per request, invoke it, and let it go out of
scope. An agent binds to its session at construction time, so a long-lived agent shared
across conversations cannot switch between them.
