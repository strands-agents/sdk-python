...
Session storage is keyed by session ID and agent ID, and a session manager assumes it
is the only writer for that key. Lost or interleaved conversation history almost always
traces back to that assumption being broken.
...
What corrupts data is two live agents addressing the same session ID and agent ID.
...
session ID and agent ID, is not detected. Every message is stored under an index that
its own manager derives from the last message it wrote, so both writers claim the same
index and overwrite each other's turns.
...
The last writer wins and the other agent's turn disappears, even though both invocations
reported success.
...
Constructing an agent is cheap. It wires up tools, hooks, and plugins in memory and
makes no model call, so building one per request costs close to nothing. Create it,
invoke it, and let it go out of scope. An agent binds to its session at construction
time, so a long-lived agent shared across conversations cannot switch between them.

<Tabs>
<Tab label="Python">

The constructor restores the session, so creating an agent with a session manager
performs one read against the storage backend.
...
Restore runs on the first invocation rather than in the constructor, so construction
itself does no I/O.
