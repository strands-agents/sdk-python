...
Nothing detects two agents in separate processes, each with its own session manager,
addressing the same session ID and agent ID. A manager seeds its message index from the
...
record when you construct it, and a record that another process just created raises
...
no model call. The model provider usually costs more, since `BedrockModel` and most other
providers build their client in the constructor, so create the provider once per process
and hand the same instance to every agent. Then build an agent per request, invoke it, and
...
pass a `boto_session` created once per process to avoid repeating that work.
