...
Session management accepts any [Storage](../storage.mdx) backend.
Choose one based on your durability needs:

| Backend | Persistence | Best for |
|---------|-------------|----------|
| <Syntax py="FileSessionManager" ts="LocalFileStorage" /> | Local disk | Development, single-machine |
| <Syntax py="S3SessionManager" ts="S3Storage" /> | Amazon S3 | Production, distributed |
