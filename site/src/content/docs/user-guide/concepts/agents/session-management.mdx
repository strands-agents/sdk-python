...
:::caution[Restored history is trusted]
An agent restores its stored messages verbatim on initialization, including any
tool-call content they carry. In the Python SDK, a tool-call block as the most
recent restored message dispatches a tool on the next invocation. If your session
store can be written by something outside your application, treat that history as
untrusted and clear tool-call content from it. See
