...
`SnapshotSessionManager` captures the whole agent as a single versioned snapshot on each turn. It replaces the message-log managers (`FileSessionManager`, `S3SessionManager`), which are deprecated. See [Migrating from the message-log managers](#migrating-from-the-message-log-managers).
</Tab>
...
Multi-agent orchestrators still use the message-log managers. `SnapshotSessionManager` covers single agents today; snapshot-based multi-agent persistence is not yet available, so `FileSessionManager` and `S3SessionManager` remain the path for Graph and Swarm.
...
`SnapshotSessionManager` covers single-agent sessions. For multi-agent orchestrators, use a message-log manager (see the multi-agent caveat above).
