...
When requests for one session ID can arrive at the same time, as with an AWS Lambda retry, queue redelivery, or a double-submitted form, serialize them before they reach the agent. Take a distributed lock keyed by the session ID before constructing the agent, release it after the invocation completes, and give it a TTL so a crashed holder cannot block the session forever. A conditional write on a DynamoDB item and `SET key value NX PX` in Redis both do the job. Where the work does not have to be synchronous, a FIFO queue partitioned by session ID (in Amazon SQS, the session ID becomes the message group ID) serializes the requests instead.

### Common Failure Modes

The built-in session managers take no distributed lock, and the single-instance invocation guard is in-process, so neither can detect a second writer running elsewhere. The patterns that corrupt history, silently in both cases:

- **Two live agents addressing the same session ID and agent ID.** The default IDs (<Syntax py='agent_id="default"' ts="id: 'agent'" />) make this easy to do by accident. Overlapping invocations overwrite each other's turns; sequential ones merge two conversations into one history. Python raises `SessionException` only when both agents share one session manager instance.
- **Reusing one session ID across concurrent executions without serializing them.** A retried AWS Lambda event or two open browser tabs each build their own agent, and both write the same keys.
