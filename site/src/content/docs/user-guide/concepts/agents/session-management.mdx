...
The TypeScript SDK uses a single `SessionManager` class paired
with a [Storage](../storage.mdx) backend:

1. **`LocalFileStorage`**: Stores snapshots on the local
   filesystem
...
### FileSessionManager / LocalFileStorage
...
Pass a `LocalFileStorage` to `SessionManager`:
...
Pass an `S3Storage` to `SessionManager`:
