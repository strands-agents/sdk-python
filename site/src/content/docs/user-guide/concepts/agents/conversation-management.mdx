...
:::caution[Deprecated]
`ConversationManager` is deprecated and will be removed in
v2. Use [ContextManager](../context-management#custom-strategies)
instead for strategy-driven context management, or pass
<Syntax py='context_manager="auto"' ts='contextManager: "auto"' />
for managed defaults.
:::
...
The Model base class provides an
<Syntax
  py="estimate_utilization(input_tokens)"
  ts="estimateUtilization(inputTokens)"
/> method that computes the fraction of the context window
consumed by a given input token count. It uses the model's
configured
<Syntax
  py="context_window_limit"
  ts="contextWindowLimit"
/> as the denominator, falling back to a default of 200,000
tokens when not set:
...
This method resolves the model's
<Syntax
  py="context_window_limit"
  ts="contextWindowLimit"
/> (falling back to the 200,000 default with a warning when
not configured) and returns `inputTokens / contextWindowLimit`.
The SDK's built-in conversation managers use this internally for
proactive compression decisions, but you can also call it
directly when building custom context management logic.
