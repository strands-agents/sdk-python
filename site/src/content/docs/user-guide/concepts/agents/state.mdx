...
agent.tool.http_request(method="GET", url="https://api.github.com/zen")
...
```
In this example we can see that the first `agent.tool.http_request()` call is
recorded in the agent's conversation history.

The second `agent.tool.http_request()` call is **not** recorded in the history
because we specified the `record_direct_tool_call=False` argument.
</Tab>
<Tab label="TypeScript">
