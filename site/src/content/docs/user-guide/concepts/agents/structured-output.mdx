...
Define an output structure using a schema and pass it to <Syntax py="structured_output_model" ts="structuredOutputSchema" />. Then, access the validated output from the `AgentResult`.
