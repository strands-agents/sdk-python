...
The token budget counts every billed token, cache reads and writes included. A cached
prompt is billed on each call, so a budget sized for an uncached workload is reached
sooner once prompt caching is enabled. See [Token
