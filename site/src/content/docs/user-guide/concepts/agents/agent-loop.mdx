...
<Tabs>
  ...
  | After tool execution | The agent stops before the next model call | Non-cooperative tools finish before cancellation is observed |
  ...
  Once a tool starts, cancellation is cooperative. Tools can check
  `tool_context.cancel_signal.is_set()` between steps or pass the event to APIs that
  accept a `threading.Event`. A tool that does neither runs to completion before the
  agent resumes cancellation handling.
  ...
  @tool(context=True)
  async def long_running_task(tool_context: ToolContext) -> str:
      for _ in range(300):
