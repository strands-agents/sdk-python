...
In the Python SDK, when you invoke an agent with a full message list rather than
plain text, that list can carry tool-call content, and a tool-call block as the
last message dispatches a tool directly on the next invocation. If the list was
built from a source you do not control, invoke with `allow_resume=False`.
See [Trusted Message History](../../safety-security/trusted-message-history.md).
:::
