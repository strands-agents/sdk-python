...
content they carry. When a snapshot could have been influenced by an untrusted
party before it reaches you, clear tool-call content from its history first, so a
restored tool-call block cannot dispatch a tool on the next invocation. See
