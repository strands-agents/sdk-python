...
content they carry. In the Python SDK, a restored tool-call block as the most
recent message dispatches a tool on the next invocation. When a snapshot could
have been influenced by an untrusted party before it reaches you, invoke the
restored agent with `allow_resume=False`. See
