...
```python
--8<-- "user-guide/concepts/agents/interventions/cedar-authorization.py:namespace"
```
</Tab>
<Tab label="TypeScript">
...
| Cedar concept | Unnamespaced (default) | Namespaced (`Agent`) |
|---|---|---|
...
The namespace must be a single Cedar identifier such as `Agent`. Multi-segment
namespaces such as `Org::App` are not supported.

:::caution
Setting `namespace` re-types the principal and resource. Migrate every unnamespaced
entity reference, including references in `when` and `unless` conditions, entity
identifiers, and any principal supplied through `principal` or
<Syntax py="principal_resolver" ts="principalResolver" />. Supplied principals are not
namespaced automatically, and the resource identifier changes from `agent` to `default`.
A stale reference in a condition fails silently, which can turn a `forbid` into a no-op.
:::
