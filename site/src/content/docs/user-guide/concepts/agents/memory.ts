...
// =====================
// Injection: enable with defaults
// =====================

function injectionDefaults() {
  const store = new BedrockKnowledgeBaseStore({
    name: 'preferences',
    config: { knowledgeBaseId: 'KB123' },
  })

  // --8<-- [start:injection_defaults]
  const agent = new Agent({
    model: new BedrockModel(),
    memoryManager: {
      stores: [store],
      injection: true, // search before each user turn, fold results into the prompt
    },
  })
  // --8<-- [end:injection_defaults]

  void agent
}
void injectionDefaults
