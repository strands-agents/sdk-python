...
**Scheduling**: Python executes in discrete batches, running up to `max_concurrency` nodes at once and waiting for the entire batch to complete before scheduling the next set of nodes. TypeScript launches nodes individually as they become ready, up to `maxConcurrency`. This avoids artificial bottlenecks where a fast node waits for a slow sibling to finish before its dependents can start.

**Node timeouts**: When `max_concurrency` is configured, `node_timeout` starts after a node acquires a concurrency slot. Time spent waiting for an available slot is not counted as node execution time.
