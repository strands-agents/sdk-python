...
builder.set_max_concurrency(2)        # Run at most 2 nodes concurrently
...
builder.set_max_concurrency(2)       # Maximum 2 concurrent node executions
...
**Scheduling**: Python executes in discrete batches, running up to `max_concurrency`
nodes at once and waiting for the entire batch to complete before scheduling the next
set of nodes. Lower limits increase batch wall-clock time. When combined with
`execution_timeout`, this can make a graph fail at the next batch boundary even if the
same graph would complete without the concurrency limit. TypeScript launches nodes
individually as they become ready, up to `maxConcurrency`.

**Node timeouts**: When `max_concurrency` is configured, `node_timeout` starts after
a node acquires a concurrency slot. Time spent waiting for an available slot is not
counted as node execution time.

:::caution
Session resume has a known fan-in issue where a node can execute twice after a restart.
Limiting concurrency makes this existing behavior easier to encounter when a sibling is
waiting for a slot. Track the separate fix in
[#3633](https://github.com/strands-agents/harness-sdk/issues/3633).
:::
