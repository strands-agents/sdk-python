...
sourceLinks:
  ...
  - path: strands-ts/src/vended-plugins/agent-tool-delegation/plugin.ts
---
...
When the orchestrator calls a delegation tool and it succeeds, the agent loop exits immediately with `stopReason: 'subagentDelegated'`. The sub-agent's response reaches the caller verbatim without reformatting.
