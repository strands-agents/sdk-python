...
sourceLinks:
  ...
  - path: strands-ts/src/vended-plugins/tool-delegation/plugin.ts
---
...
## Delegation: Direct Response Routing
...
The `delegate` option solves this. When you mark an agent tool with `delegate: true`, the orchestrator returns the sub-agent's response directly to the caller without an additional model call.

```mermaid
flowchart LR
    User([User]) --> Orchestrator["Orchestrator Agent"]
    Orchestrator -->|delegate| Billing["Billing Agent"]
    Orchestrator -->|delegate| TechSupport["Tech Support Agent"]
...
When the orchestrator calls a delegation tool and it succeeds, the agent loop exits immediately with `stopReason: 'delegated'`. The sub-agent's response reaches the caller verbatim — no reformatting, no extra model call.

### Mixing Delegation and Regular Tools

Delegation tools coexist with regular tools on the same orchestrator. Regular tool results feed back into the orchestrator for further reasoning; delegation results short-circuit the loop.
