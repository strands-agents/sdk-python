...
When the orchestrator calls a delegation tool and it succeeds, the agent loop exits immediately with `stopReason: 'toolUse'`. The sub-agent's response reaches the caller verbatim without reformatting.
