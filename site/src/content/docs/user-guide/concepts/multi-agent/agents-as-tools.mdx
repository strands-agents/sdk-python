...
sourceLinks:
  ...
  - path: strands-ts/src/vended-plugins/direct-return/plugin.ts
---
...
The `handoff` option solves this. When you mark an agent tool with `handoff: true`, the orchestrator returns the sub-agent's response directly to the caller without an additional model call.
...
When the orchestrator calls a handoff tool and it succeeds, the agent loop exits immediately with `stopReason: 'directReturn'`. The sub-agent's response reaches the caller verbatim — no reformatting, no extra model call.
