...
sourceLinks:
  ...
  - path: strands-ts/src/vended-plugins/handoff/plugin.ts
---
...
Strands Agents SDK provides three ways to implement the "Agents as Tools" pattern: passing agents directly in the `tools` array for the simplest setup, <Syntax py=".as_tool()" ts=".asTool()" /> when you need to customize tool name, description, or context behavior, and the `@tool` decorator or `tool()` function for full control over how the agent is invoked.
...
When you need to customize the tool name, description, or context behavior, use <Syntax py=".as_tool()" ts=".asTool()" /> explicitly:
...
By default, both direct passing and <Syntax py=".as_tool()" ts=".asTool()" /> reset the agent's conversation context between invocations, ensuring every call starts from a clean baseline. To preserve the agent's conversation history across invocations:
...
## Handoff: Direct Response Routing

In many routing scenarios, the orchestrator's only job is to pick the right specialist — once the specialist responds, there's nothing for the orchestrator to add. By default, the orchestrator makes an additional model call to post-process the sub-agent's response, which adds latency, token cost, and risks paraphrasing structured output.

The `handoff` option solves this. When you mark an agent tool with <Syntax py="handoff=True" ts="handoff: true" />, the orchestrator returns the sub-agent's response directly to the caller without an additional model call.

```mermaid
flowchart LR
    User([User]) --> Orchestrator["Orchestrator Agent"]
    Orchestrator -->|handoff| Billing["Billing Agent"]
    Orchestrator -->|handoff| TechSupport["Tech Support Agent"]
    Billing -->|direct response| User
    TechSupport -->|direct response| User
```
...
```typescript
--8<-- "user-guide/concepts/multi-agent/agents-as-tools.ts:handoff_basic"
```
</Tab>
</Tabs>

When the orchestrator calls a handoff tool and it succeeds, the agent loop exits immediately with <Syntax py="stop_reason='handoff'" ts="stopReason: 'handoff'" />. The sub-agent's response reaches the caller verbatim — no reformatting, no extra model call.

### Mixing Handoff and Regular Tools

Handoff tools coexist with regular tools on the same orchestrator. Regular tool results feed back into the orchestrator for further reasoning; handoff results short-circuit the loop.
...
orchestrator = Agent(
    tools=[
        calculator,                              # regular tool — results feed back to orchestrator
        researcher.as_tool(handoff=True),         # handoff — response goes directly to user
    ],
)
```
</Tab>
...
```typescript
--8<-- "user-guide/concepts/multi-agent/agents-as-tools.ts:handoff_mixed"
```
</Tab>
</Tabs>
