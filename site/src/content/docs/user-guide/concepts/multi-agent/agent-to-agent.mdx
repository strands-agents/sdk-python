...
result = a2a_agent("What is the current temperature in Seattle?")
print(result.message)
# Typical output:
# {'role': 'assistant', 'content': [{'text': 'It is 23.1°C in Seattle right now.'}]}
```
...
async def main():
    a2a_agent = A2AAgent(endpoint="http://localhost:9000")
    result = await a2a_agent.invoke_async("Is it warmer in Tokyo or Seattle today?")
    print(result.message)
...
const result = await a2aAgent.invoke('Is it warmer in Tokyo or Seattle today?')
...
weather_agent = A2AAgent(
    endpoint="http://weather-service:9000",
    name="weather"
)
...
def get_weather(location: str) -> str:
    ...
    result = weather_agent(f"What is the current weather in {location}?")
    return str(result.message["content"][0]["text"])

orchestrator = Agent(
    system_prompt="You are a helpful assistant. Use the get_weather tool for weather.",
    tools=[get_weather]
)
```
...
# The agent will be accessible at http://my-alb.amazonaws.com/weather/
a2a_server = A2AServer(
    agent_factory=create_agent,
    http_url="http://my-alb.amazonaws.com/weather"
)
...
a2a_server_with_root = A2AServer(
    agent_factory=create_agent,
    http_url="http://my-alb.amazonaws.com/weather",
    serve_at_root=True  # Serves at root even though URL has /weather path
)
```
...
const server = new A2AExpressServer({
  agentFactory: (contextId) => new Agent({ systemPrompt: 'A weather agent.' }),
  name: 'Weather Agent',
  httpUrl: 'http://my-alb.amazonaws.com/weather',
})

const express = (await import('express')).default
const app = express()
app.use('/weather', server.createMiddleware())
app.listen(9000)
```
