...
- **Pre-launch cancel**: set `BeforeToolCallEvent.cancel_tool` on a per-tool hook to produce an error result for that tool, or set `BeforeToolsEvent.cancel` on the batch-level hook to produce error results for every tool in the batch before any execute.
