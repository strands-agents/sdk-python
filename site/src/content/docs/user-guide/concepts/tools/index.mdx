...
agent = Agent(
    tools=[calculator, file_read, bash]
)
...
# Agent will use the bash and file reader tool when appropriate
...
```typescript
--8<-- "user-guide/concepts/tools/tools_imports.ts:direct_invocation_imports"
...
Agents can be passed directly in another agent's `tools` array — the SDK automatically converts them into tools. Use <Syntax py=".as_tool()" ts=".asTool()" /> when you need to customize the tool name, description, or context behavior. For full details, see [Agents as Tools](../multi-agent/agents-as-tools.md).
...
[Function-Based Tools](#function-based-tools) above for how `@tool` works.
