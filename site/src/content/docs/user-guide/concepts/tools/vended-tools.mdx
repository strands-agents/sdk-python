...
- **`agentic`** (default) — raw page content is passed to an analyst agent that
  answers a `prompt`, so the full page never enters the main agent's context.
  Use when targeted answers are needed about potentially large pages.
...
timeouts, redirects, proxies, or caching. For `mode='agentic'`, the factory
also accepts a `model` for the analyst; the agent's own model is used when
none is supplied.
