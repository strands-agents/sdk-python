...
Fetches an HTTP(S) URL and returns its content as markdown. HTML pages are
converted to markdown with scripts, styles, and noise stripped. Distinct from
the HTTP Request tool, which returns raw response bodies for API calls.

When the model passes a `prompt` alongside the URL, the page is processed by a
summarizer sub-agent that answers the prompt, so only the answer — not the full
page — enters the main agent's context. The agent's own model is used by
default.
