...
The `max_bytes` parameter caps the HTTP response size (default 5 MiB);
`max_content_chars` caps the extracted content delivered to the model or
...
:::note[TypeScript: install required]
...
```typescript
--8<-- "user-guide/concepts/tools/vended-tools-imports.ts:web_fetch_import"

--8<-- "user-guide/concepts/tools/vended-tools.ts:web_fetch_example"
```

Reading the full page as markdown:

```typescript
--8<-- "user-guide/concepts/tools/vended-tools-imports.ts:web_fetch_markdown_import"

--8<-- "user-guide/concepts/tools/vended-tools.ts:web_fetch_markdown_example"
```
