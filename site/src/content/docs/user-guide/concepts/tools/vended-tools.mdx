...
`web_fetch` accepts only `http://` and `https://` URLs and caps response bodies at five mebibytes. Egress control belongs at the layer that encapsulates the agent — a sandbox, microVM, or network policy — where it can be enforced consistently across all tools, including `shell`.
:::
