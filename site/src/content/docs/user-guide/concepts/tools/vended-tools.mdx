...
sourceLinks:
  ...
  - path: strands-ts/src/vended-tools/swarm/swarm.ts
  - path: strands-py/src/strands/vended_tools/swarm/swarm.py
---
...
| [Swarm](#swarm) | Spin up a handoff-based sub-agent team at runtime | Python, TypeScript (Node.js, browsers) |
...
Lets the agent spin up a small handoff-based sub-agent team at runtime. The parent supplies a list of sub-agent specs (name, system prompt, tool allowlist) and an initial input, and the tool builds those agents, wires them into a swarm, and returns the final response along with which agents ran.

_Supported in: Node.js, modern browsers (TypeScript); all platforms (Python)._

Sub-agents inherit the parent's model and can only use tools whose names appear in the parent's tool registry. The tool participates in a shared multi-agent recursion counter with a default cap of three, so nested calls through peer multi-agent tools cannot bypass the cap. A default limit of five sub-agents per invocation, a total wall-clock ceiling of three hundred seconds, and a per-node ceiling of one hundred twenty seconds are all set at the tool boundary and not model-configurable. Use the make_swarm factory (Python) or makeSwarm (TypeScript) at tool construction to tighten the envelope; the model still cannot alter these caps at call time.
...
```typescript
--8<-- "user-guide/concepts/tools/vended-tools-imports.ts:swarm_import"

--8<-- "user-guide/concepts/tools/vended-tools.ts:swarm_example"
```
...
agent("Use a swarm to research and summarize recent octopus news.")
```

</Tab>
</Tabs>

[Full API Reference](https://github.com/strands-agents/harness-sdk/blob/main/strands-ts/src/vended-tools/swarm/README.md)
