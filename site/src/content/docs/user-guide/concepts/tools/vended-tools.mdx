...
sourceLinks:
  ...
  - path: strands-ts/src/vended-tools/use-agent/use-agent.ts
  - path: strands-py/src/strands/vended_tools/use_agent/use_agent.py
---
...
| [Use Agent](#use-agent)       | Delegate a single task to a nested agent        | Python, TypeScript (Node.js, browsers)         |
...
Delegates a single task to a freshly constructed nested agent. The calling agent supplies a system prompt, an explicit allowlist of parent-registered tool names to expose to the child, and the task itself. The child inherits the parent's model instance, runs with a fresh conversation, and returns its final assistant text.

_Supported in: Node.js, modern browsers (TypeScript); all platforms (Python)._

Delegation is a privilege-escalation surface. The model chooses the child's tools from the parent's registry (wildcards and multi-agent tool names are rejected) and spends inference tokens on the parent's account. The model cannot choose a different model provider or supply provider-specific settings; the child always inherits the parent's model instance. Nested delegation is capped at depth three, shared across the multi-agent vended tools, and cannot be raised by the model.
...
agent("Delegate a sub-task to a researcher agent.")
```

</Tab>
</Tabs>

The tool returns an object with a `status`, the child agent's final text as `output`, and the total wall-clock in `executionTimeMs` (TypeScript) or `execution_time_ms` (Python). `status` uses the shared multi-agent vocabulary of `completed`, `failed`, or `interrupted`. Only a child that finishes with `end_turn` maps to `completed`; any other non-cancelled stop reason maps to `failed` so the parent can distinguish a completed delegation from one that hit a limit or policy. On cancellation the TypeScript tool raises an `AbortError`, matching the sibling http-request tool; the Python tool returns `status: "cancelled"`.

[Full API Reference](https://github.com/strands-agents/harness-sdk/blob/main/strands-ts/src/vended-tools/use-agent/README.md)
