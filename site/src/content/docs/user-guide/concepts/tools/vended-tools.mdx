...
Fetches an HTTP(S) URL and returns its content. Supports two modes: `markdown`
returns the page as clean markdown with scripts, styles, and noise stripped;
`agentic` passes the raw content to an analyst agent that answers a `prompt`
so the full page never enters the main agent's context. Distinct from the HTTP
Request tool, which returns raw response bodies for API calls.
...
timeouts, redirects, proxies, caching, or other transport-level configuration.
For `mode='agentic'`, the factory also accepts a `model` for the analyst; the
...
Custom configuration — custom HTTP client, response size cap, and a dedicated
summarizer model:
