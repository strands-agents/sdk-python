...
```typescript
--8<-- "user-guide/concepts/tools/mcp-tools.ts:progress_notifications"
```

The callback receives the MCP progress object, including `progress`, and optional `total` and `message` values. Progress callbacks are currently ignored when experimental task-augmented execution is enabled.
