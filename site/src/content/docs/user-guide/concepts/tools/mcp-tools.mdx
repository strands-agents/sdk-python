...
Use the client APIs to discover prompts and resources, retrieve a prompt with arguments, or read resource content. List responses include the server's `nextCursor` when another page is available.
...
resource = mcp_client.read_resource_sync("file:///reports/q1.md")
...
These methods return the raw MCP result objects. They do not convert prompt messages into agent conversation history.
