...
flowchart LR
    ...
    style runtime fill:#1a1a1a,stroke:#555,stroke-width:2px,color:#ccc
    style sb fill:#1a1a1a,stroke:#555,stroke-width:2px,color:#ccc
    style M fill:#2a2a2a,stroke:#666,color:#ddd
    style L fill:#2a2a2a,stroke:#666,color:#ddd
    style T fill:#2a2a2a,stroke:#666,color:#ddd
    style E fill:#2a2a2a,stroke:#666,color:#ddd
```
...
| Method | Description |
| --- | --- |
| `executeStreaming` / `execute_streaming` | Run a shell command, stream output |
| `executeCodeStreaming` / `execute_code_streaming` | Run code via an interpreter, stream output |
| `readFile` / `read_file` | Read a file as bytes |
| `writeFile` / `write_file` | Write bytes to a file |
| `removeFile` / `remove_file` | Delete a file |
| `listFiles` / `list_files` | List directory contents |
...
Custom sandbox implementations can also override `getTools()` / `get_tools()` to vend their own tools entirely.
