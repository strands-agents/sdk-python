...
- **The breakpoint moves with the conversation instead of accumulating.** Each request carries exactly
  one message breakpoint, on the newest turn, so a long session never approaches the API's limit of
  four breakpoints per request. Cache points you place by hand in the messages are replaced while
  `cache_config` is set, because it owns their placement.
...
- **`cache_tools` works on its own.** It does not require `cache_config`, so tool definitions can be
  cached without caching the conversation.
