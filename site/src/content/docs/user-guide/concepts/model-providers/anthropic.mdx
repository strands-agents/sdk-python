...
- When Anthropic pauses a long-running server-tool turn, the model provider resumes it automatically (up to 5 times per request) before the response reaches the agent loop.
