...
- Server tools are not sent when a specific tool call is forced (`tool_choice` of `any` or `tool`), which includes structured output; the model can only use them on turns where it is free to choose.
