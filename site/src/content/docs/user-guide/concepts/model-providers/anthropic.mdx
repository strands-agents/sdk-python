...
```typescript
--8<-- "user-guide/concepts/model-providers/anthropic_imports.ts:builtin_tools_imports"

--8<-- "user-guide/concepts/model-providers/anthropic.ts:builtin_tools"
```
</Tab>
</Tabs>

Web search results are surfaced as citations on the response text. For available built-in tools and their versioned `type` strings, see the [Anthropic tool use documentation](https://platform.claude.com/docs/en/docs/agents-and-tools/tool-use/overview).
...
- If Anthropic pauses a long-running server-tool turn, the agent stops with a <Syntax py="pause_turn" ts="pauseTurn" /> stop reason and the response may be partial or empty.
