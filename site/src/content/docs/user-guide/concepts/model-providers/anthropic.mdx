...
In Python, set `cache_config` to add a cache breakpoint to the last user message, which caches
everything before it, and `cache_tools` to cache the tool definitions. In TypeScript, a single
`cacheConfig` covers both sections, and either can be turned off on its own:
...
| Strategy | Meaning |
|----------|---------|
| `"auto"` (default) | Cache normally. On providers that serve non-Claude models this also checks that the selected model supports caching. |
| `"anthropic"` | Cache normally, skipping that support check. |
...
`CacheConfig` can move between providers unchanged, and `strategy` can be omitted entirely.

#### Choosing what to cache (TypeScript)

`cacheConfig` caches both the tool definitions and the conversation. Each section is on by default,
so you opt a section *out* rather than in. `toolsTTL` and `messagesTTL` take either a TTL for that
section or `false` to turn it off:
...
| `{ ttl: '1h', messagesTTL: '5m' }` | both, tools at one hour and the conversation at five minutes |

Turning both sections off is the same as leaving `cacheConfig` unset.
...
- **Without a cache point of your own, the breakpoint goes on the last block of the newest turn that can
  ...
  would go stale.
- **Caching a section is not free of the four-breakpoint limit.** The API accepts at most four cache
  breakpoints per request, shared across tools, system and messages. Automatic placement contributes
  one per enabled section, so hand-placing several of your own alongside it can exceed the limit and
  the API rejects the request.
- **`ttl` defaults to 5 minutes; `"1h"` costs more to write.** See
  [pricing](https://docs.claude.com/en/docs/build-with-claude/prompt-caching#pricing). An empty `ttl`
  counts as unset and falls back to the default rather than being sent.
- **In Python, `cache_tools` works on its own.** It does not require `cache_config`, so tool
  definitions can be cached without caching the conversation. Passing a plain string (`"default"`)
  only switches it on — the value is not a TTL. Use `CacheToolsConfig(ttl=...)` to set one.
- **In TypeScript, one `cacheConfig` moves between providers unchanged.** `AnthropicModel` and
  `BedrockModel` accept the same shape with the same meaning, so a configuration can be reused as-is
