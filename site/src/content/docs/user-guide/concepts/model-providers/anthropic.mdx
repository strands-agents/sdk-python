...
Anthropic reports prompt cache reads in `cacheReadInputTokens` and cache writes in
`cacheWriteInputTokens`. Both are counted separately from `inputTokens`, and all four
counters sum to `totalTokens`. Thinking tokens appear in `reasoningOutputTokens`, which
reports how many of the billed output tokens were internal reasoning and is therefore part
of `outputTokens` rather than additional to it. See [Token
