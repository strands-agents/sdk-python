...
`cache_config` caches the system prompt and the conversation; set `system_ttl=False` to
leave the system prompt uncached, or a TTL string (`system_ttl="1h"`) to give it its own
