...
To track spend for a LiteLLM-backed agent, read `cost` from the result of each
invocation. The value includes every model call made during that invocation,
including additional calls after tool use.
...
result = agent("Summarize the benefits of prompt caching.")

if result.cost is not None:
    print(f"Invocation cost: ${result.cost:.6f}")
```

LiteLLM calculates the estimate from its model pricing data. `cost` is `None`
when LiteLLM cannot price the selected model. If one model call cannot be priced,
the value is a lower bound for the invocation.
