...
Writer reports prompt cache reads in `cacheReadInputTokens`, counted separately from
`inputTokens`, and reasoning tokens in `reasoningOutputTokens`, which is a subset of
`outputTokens` rather than additional to it. See [Token
