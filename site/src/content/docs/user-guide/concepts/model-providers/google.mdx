...
Google reports cached content in `cacheReadInputTokens` and thinking tokens in
`reasoningOutputTokens`. Cached tokens are counted separately from `inputTokens`, and
tokens from tool results count as input. See [Token
