...
- `cacheConfig` - Enable prompt caching, e.g. `{ ttl: '1h' }` or `{ messagesTTL: false }` for tool definitions only
...
Bedrock requires checkpoint TTLs to be non-increasing across `toolConfig`, system and messages, and rejects a longer TTL that follows a shorter one. A system prompt cache point you place by hand therefore takes the TTL configured on `cacheConfig`/`cache_config` when it carries none of its own, so a single `ttl` keeps every checkpoint in step.

The configured TTL stands down in two cases, leaving the point at Bedrock's default of 5 minutes: when you wrote a TTL on the point yourself, which is used as written; and when the tools TTL differs from the shared one at all, whether shorter or longer. Two TTLs in tension are yours to reconcile. A falsy TTL is not a TTL - an empty or null `ttl` on a cache point you placed is dropped rather than sent, since Bedrock validates it against an enum.
...
In TypeScript, tool caching is enabled through `cacheConfig`. When `cacheConfig` is set, the SDK automatically appends a cache point after the tool definitions in each request. There is no separate `cacheTools` option — one `cacheConfig` covers both the tool definitions and the conversation, and each section can be turned off on its own.

Both sections are cached by default. `toolsTTL` and `messagesTTL` control them individually: pass a TTL to set that section's duration, or `false` to turn it off. So `cacheConfig: { messagesTTL: false }` caches the tool definitions only, and `cacheConfig: { toolsTTL: false }` caches the conversation only. A `ttl` on `cacheConfig` applies to both sections, and a per-section TTL overrides it.
...
The two compose: with `cache_config` set, a cache point you place yourself in the last user message is kept where you put it, and automatic placement steps aside. That matters when your last user message ends with content rebuilt on every call — retrieved context, a timestamp, injected state — because a cache point after that content puts it inside the cached prefix, so every request writes a new cache entry and none ever reads one. Place the cache point ahead of the per-call content to keep the durable prefix cacheable. Your point has to have something ahead of it to cache: a point placed as the first block of the message, or with only documents ahead of it, is dropped, because Bedrock rejects a cache point with nothing before it; automatic placement then applies its own rules, which for a leading document means no cache point at all. Cache points in earlier messages are still removed, so they cannot accumulate one per turn against the provider's cache-point budget.
