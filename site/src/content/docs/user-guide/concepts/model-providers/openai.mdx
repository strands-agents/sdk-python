...
```typescript
--8<-- "user-guide/concepts/model-providers/openai_imports.ts:structured_output_imports"

--8<-- "user-guide/concepts/model-providers/openai.ts:structured_output"
```
</Tab>
</Tabs>
...
OpenAI reports prompt cache reads in `cacheReadInputTokens`, cache writes in
`cacheWriteInputTokens` for GPT-5.6 and later, and reasoning tokens in
`reasoningOutputTokens`. The cache counters are separate from `inputTokens` rather than
included in it. See [Token
