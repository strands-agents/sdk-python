...
async for event in agent.stream_async(
    "What is the capital of France? Then fetch https://api.open-meteo.com/v1/forecast"
    "?latitude=48.85&longitude=2.35&current=temperature_2m"
    " and tell me the temperature there."
):
    process_event(event)
...
@tool
async def weather_agent(query: str) -> AsyncIterator:
    """Answer weather questions using the http_request tool."""
    agent = Agent(
        name="Weather Assistant",
        system_prompt="You are a weather assistant. Use http_request for live data.",
...
orchestrator_async_iterator = Agent(
    system_prompt="Route weather questions to the weather_agent tool.",
    callback_handler=None,
    tools=[weather_agent]
)
...
orchestrator_callback("What is the temperature in Tokyo right now?")
```
</Tab>
<Tab label="TypeScript">
