...
- **`event_loop_throttled_delay`**: Delay, in seconds, before the agent loop retries a
  model call after model provider throttling. `ModelRetryStrategy` emits this event for
