...
- **`event_loop_throttled_delay`**: Delay, in seconds, that the agent loop waited before retrying a model call after a retryable failure — by default only throttling, see [Retry Strategies](../agents/retry-strategies.md). Emitted after the wait completes, for backward compatibility.
