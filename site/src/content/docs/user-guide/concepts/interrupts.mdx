...
- When a tool batch is interrupted, `AfterToolsEvent` fires with the results collected so far. The interrupt remains the terminal outcome even if a callback sets `end_turn`.
</Tab>
<Tab label="TypeScript">
