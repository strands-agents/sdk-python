...
class BatchApprovalHook(HookProvider):
    ...
    def approve(self, event: BeforeToolsEvent) -> None:
        tool_names = [
            content["toolUse"]["name"] for content in event.message["content"] if "toolUse" in content
        ]
        approval = event.interrupt(f"{self.app_name}-batch-approval", reason={"tools": tool_names})
        if approval.lower() != "y":
            event.cancel = "User denied permission to run the requested tools"
