...
| <Syntax py="citation_document_blocks" ts="citationDocumentBlocks" /> | When set to `true`, exposes a `retrieve_knowledge_base` tool that returns retrieved passages as `DocumentBlock` objects with citations enabled. Renders passage attribution citations natively via Bedrock Converse API. |
| <Syntax py="retrieve_tool_config" ts="retrieveToolConfig" /> | Custom name and description for the retrieval tool exposed when <Syntax py="citation_document_blocks" ts="citationDocumentBlocks" /> is enabled. |
...
When using Bedrock Knowledge Bases for RAG, you can enable native attribution citations generated by the Amazon Bedrock Converse API. Set <Syntax py="citation_document_blocks" ts="citationDocumentBlocks" /> to `true` when constructing the store. This exposes a dedicated `retrieve_knowledge_base` tool via `getTools()`.

When the agent invokes the retrieval tool, each retrieved text chunk is surfaced to the model as a structured `DocumentBlock` with citations enabled. Bedrock uses these blocks to automatically attach citation references to the assistant's generated response.
...
// Wire into MemoryManager (disable automatic search prompt injection so the agent uses the tool directly)
...
You can customize the tool's name and description by passing <Syntax py="retrieve_tool_config" ts="retrieveToolConfig" /> (`{ name: '...', description: '...' }`).
