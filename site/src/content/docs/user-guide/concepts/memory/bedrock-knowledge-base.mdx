...
| <Syntax py="min_score" ts="minScore" /> | Floor a result must meet. Use for similarity-scored knowledge bases, where a higher score is a better match. See [Filtering by score](#filtering-by-score). |
| <Syntax py="max_score" ts="maxScore" /> | Ceiling a result must not exceed. Use for distance-scored knowledge bases, where a lower score is a better match. See [Filtering by score](#filtering-by-score). |
...
Set <Syntax py="min_score" ts="minScore" /> or <Syntax py="max_score" ts="maxScore" /> on the store to drop weakly-matching results. The two are mutually exclusive, and setting both raises at construction. Which one applies depends on how the knowledge base scores: a similarity-scored knowledge base treats a higher score as the better match, so <Syntax py="min_score" ts="minScore" /> keeps results at or above the floor; a distance-scored one treats a lower score as the better match, so <Syntax py="max_score" ts="maxScore" /> keeps results at or below the ceiling. A vector knowledge base backed by Aurora PostgreSQL with pgvector's `<=>` operator returns cosine distance and needs the ceiling.

The metric is a property of the vector store behind the knowledge base, which Retrieve does not report, so it cannot be detected automatically — choosing the wrong field silently keeps the worst matches and discards the best. The value is compared against the raw score the knowledge base reports and is not range-checked, so it sits on whatever scale that backend uses.

Filtering happens after retrieval, since Retrieve has no score-bound parameter. A search may therefore return fewer entries than <Syntax py="max_search_results" ts="maxSearchResults" />, or none at all when nothing clears the bound. A result the knowledge base did not score is kept, as there is nothing to compare it against.
