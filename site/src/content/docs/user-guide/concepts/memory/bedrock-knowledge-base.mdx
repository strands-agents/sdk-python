...
When using Bedrock Knowledge Bases for RAG, you can enable native passage-level citations generated by the Amazon Bedrock Converse API. Build a custom `FunctionTool` that retrieves passages from the store and wraps each one in a `DocumentBlock` with `citations: { enabled: true }`. When the model receives these blocks in a tool result, Converse generates citation references automatically.

This uses the existing [`MemoryStore.getTools()`](./overview#stores) extension point and `tools` on the agent — no SDK config changes required:
...
// Build a retrieve tool that returns DocumentBlocks with citations enabled
const retrieveTool = new FunctionTool({
  name: 'retrieve_knowledge_base',
  description: 'Search the knowledge base and return cited document passages.',
  inputSchema: {
    type: 'object',
    properties: {
...
// Disable the built-in search_memory tool, pass the custom retrieve tool alongside
const agent = new Agent({
  model: new BedrockModel(),
  memoryManager: new MemoryManager({ stores: [store], searchToolConfig: false }),
  tools: [retrieveTool],
})
```

See the [`kb-native-citations` example](https://github.com/strands-agents/sdk-typescript/tree/main/strands-ts/examples/kb-native-citations) for a complete, runnable version.
