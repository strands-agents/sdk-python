...
```shell
pip install 'strands-agents[bidi-bedrock,otel]'
```
:::
...
| Google Live | None | Emits no response lifecycle events, so a session produces no `bidi_response` spans. Interruptions, token usage including cache reads, and the modality breakdown are still reported. |
...
| No `bidi_response` spans | Expected on Google Live, which emits no response lifecycle |
