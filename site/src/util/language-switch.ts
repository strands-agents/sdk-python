 ...
 *   2. Otherwise fall back to the target language's section index
 *      (e.g. `docs/api/typescript`), if it exists.
 *   3. Otherwise don't navigate at all.
 */
 ...
 * @returns A root-relative path with a trailing slash, or null if no navigation should happen
 *   (the page has no language segment, is already in the target language, or has no counterpart)
 */
