...
- Use `<Syntax py="python_name" ts="tsName" />` for inline language-specific identifiers in prose (auto-imported). Renders as `<code>` by default; pass `code={false}` for plain text. Reacts live to the global language toggle.
