...
class NovaSonicSession(BidirectionalModelSession):
    ...
    def _build_initialization_events(self, system_prompt: str, tools: List[ToolSpec], 
    ...
    def _get_session_start_event(self) -> str:
        ...
        return json.dumps({
            "event": {
                "sessionStart": {
                    "inferenceConfiguration": NOVA_INFERENCE_CONFIG
                }
            }
        })
    ...
    def _build_tool_configuration(self, tools: List[ToolSpec]) -> List[Dict]:
        ...
        tool_config = []
        for tool in tools:
            input_schema = ({"json": json.dumps(tool['inputSchema']['json'])} 
...
class NovaSonicBidirectionalModel(BidirectionalModel):
    ...
    async def create_bidirectional_connection(
        ...
        **kwargs
        ...
        try:
            stream = await time_it_async("invoke_model_with_bidirectional_stream", 
