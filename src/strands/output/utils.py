...
def resolve_output_schema(
    structured_output_model: Optional[Union[Type[BaseModel], OutputSchema]] = None,
    output_mode: Optional[OutputMode] = None,
) -> Optional[OutputSchema]:
    """Resolve output type and mode into OutputSchema.

    Args:
        structured_output_model: Output type specification
    ...
    if isinstance(structured_output_model, OutputSchema):
        return structured_output_model

    if output_mode is None:
        output_mode = ToolMode()

    return OutputSchema(structured_output_model, output_mode)
