...
class Agent:
        ...
        *,
        ...
        Args:
            ...
            sub_agents: List of sub-agents available for delegation.
                Each Sub-agent will have a corresponsind handoff_to_{name} tool autogenerated for complete delegation.
            delefation_timeout: Timeout in seconds for delegation operations.
                default to 300seconds or 5 minutes, Set to None for no Timeout, 
            delegation_state_transfer: Whether to transfer agent.state to sub_agents.
                defaults to true, when true, sub-agents recieves a deep copy of the orchestrators's state.
            delegation_message_transfer: whether to transfer conversation history.
                defaults to True. Contorls wherer messages are copied to sub-agent.
            max_delegation_depth: Maximum allowed depth for nested delefation, prevents infinite delegation chains, defaults to 10\
            delegation_state_serializer: Optinal custom serializer for state transfer. 
                when provided this callable will be used to serialize ste instead of deepcopy. 
                this is useful for large or complex states where deepcopy is inefficient/not a good use of context windows.
                should return a serialized copy of the state
            delegation_streaming_proxy: wherther to proxy streaming events from subagemts. it defaults to true, 
        ...
        # Initialization of the sub-agents and delefation configuration
    ...
    def add_sub_agents(self,agent:"Agent")->None:
        """Add a new sub-agents dynamically

        Args:
            agent: Agent to add as a sub-agent
        Raises: 
            ValuesErrorL If agent validation fails
        """
    ...
    def remove_sub_agent(self,agent_name:str)->bool:
        ...
        Args: 
            agents_name: Name of the sub-agent to remove
        ...
        if agent_name in self._sub_agents:
            ...
            #remove delegation tool from the registery
            tool_name=f"hanfoff_to_{agent_name.lower().replace('-','_')}"
    ...
    def _generate_delegation_tools(self,sub_agents:list["Agent"])->None:
        ...
        Args:
            sub_agents : List of sub-agents to generate tools 
        """
        ...
        for sub_agent in sub_agents:
            ...
            def delegation_tool(
                ...
                """Transder control completely to specified sub_agent.
                    this tool completely delegates the current request to the target agent.
                    the orchesttrator will terminate and the sub_agent's responsse will become
                    the final respoonse with no additional processing.
                    

                Args:
                    message (str): Message to pass to the target agent
                    context (dict[str,Any] | None, optional): Additional context transfer Defaults to None.
                    transfer_state (bool | None, optional): Ovveride the default state transfer(Optional) behaviour Defaults to None.
                    transfer_messages (bool | None, optional):Ovveride the default message transfer (Optional)Defaults to None.
                    _target_agent (str, optional): Internal target agent identifier. Defaults to sub_agent.name.
                    _delegation_chain (list[str] | None, optional): Internal delegation Tracking. Defaults to None.

                Returns:
                    dict[str,Any]: This tool raises AgentDelegationException and does not return normally.
                """
