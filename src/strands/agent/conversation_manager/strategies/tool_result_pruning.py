...
from ..pruning_conversation_manager import MessageContext, PruningStrategy
...
class LargeToolResultPruningStrategy(PruningStrategy):
    ...
    def __init__(
        ...
        text_truncation_threshold: int = 500,
    ):
        ...
        Args:
            ...
            text_truncation_threshold: Character length threshold for truncating text content
        """
        ...
        self.text_truncation_threshold = text_truncation_threshold
    ...
    def _simple_compress_tool_result(self, tool_result: ToolResult) -> ToolResult:
        ...
        for content_item in tool_result.get("content", []):
            if "text" in content_item:
                text = content_item["text"]
                if len(text) > self.text_truncation_threshold:  # Truncate long text
                    compressed_text = text[: self.text_truncation_threshold] + f"... [truncated from {len(text)} chars]"
