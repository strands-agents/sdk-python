...
class CategorizedMemoryItem:
    ...
    def should_demote(self, inactive_threshold: float) -> bool:
        ...
        return (self.category == MemoryCategory.ACTIVE and 
                self.idle_time() > inactive_threshold)
    
    def should_archive(self, archive_threshold: float) -> bool:
        """Check if item should be archived."""
        return (self.category == MemoryCategory.CACHED and 
                self.age() > archive_threshold)
...
class MemoryLifecycleManager:
    ...
    def get_item(self, key: str) -> Optional[Any]:
        ...
        if key in self._items:
            ...
            if (item.category == MemoryCategory.CACHED and 
                self._should_promote(item)):
    ...
    def cleanup_memory(self, force: bool = False) -> int:
        ...
        for item in list(self._items.values()):
            if item.should_demote(self.config.thresholds.cache_ttl):
        ...
        if self.config.enable_archival:
            for key, item in list(self._items.items()):
                if item.should_archive(self.config.thresholds.archive_after):
        ...
        if force or total_size > self.config.thresholds.total_memory_limit:
    ...
    def _check_and_cleanup(self) -> None:
        ...
        total_size = sum(item.size for item in self._items.values())
        utilization = total_size / self.config.thresholds.total_memory_limit
        
        if utilization >= self.config.thresholds.cleanup_threshold:
            self.cleanup_memory()
    ...
    def _emergency_cleanup(self) -> int:
        ...
        items_by_priority = sorted(
            self._items.items(),
            key=lambda x: (x[1].category.value, x[1].last_accessed)
        )
        
        # Calculate how many items to remove
        total_items = len(self._items)
        target_removal = max(1, int(total_items * self.config.thresholds.emergency_cleanup_ratio))
    ...
    def optimize_memory(self) -> Dict[str, int]:
        ...
        return {
            ...
            "size_reduction_pct": ((initial_size - final_size) / initial_size * 100) if initial_size > 0 else 0
        }