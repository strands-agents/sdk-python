...
class EnhancedToolFormatter:
    ...
    def display_tool_call(self, tool_name: str, tool_input: dict, agent_name: str = "my_agent") -> None:
        ...
        if self.use_colors:
            params = (
                f"{self.COLORS['cyan']}Parameters:{self.RESET} "
                f"{self.COLORS['white']}{params_str}{self.RESET}"
            )
        else:
    ...
    def format_tool_completion(self, tool_name: str, success: bool = True,
                             result: Any = None, error: str | None = None, 
                             formatted_tool_name: str | None = None) -> str:
        ...
        if success:
            colored_header = f"{self.COLORS['green']}{action}{self.RESET} | {self.COLORS['gray']}{display_name}{self.RESET} - {self.COLORS['white']}{timestamp} - status={status}{self.RESET}"
        else:
            colored_header = f"{self.COLORS['red']}{action}{self.RESET} | {self.COLORS['gray']}{display_name}{self.RESET} - {self.COLORS['white']}{timestamp} - status={status}{self.RESET}"
