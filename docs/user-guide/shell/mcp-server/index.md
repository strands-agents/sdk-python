...
Run the server with the `--mcp` flag. With no config, it starts a bare in-memory sandbox without host files or credentials and uses the [default network policy](/docs/user-guide/shell/configuration/index.md#network-access).
