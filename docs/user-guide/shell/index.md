...
Strands Shell is a Bourne-compatible shell that runs inside your own process. It ships `grep`, `sed`, `jq`, `curl`, `find`, and dozens of other commands without calling `fork`, `exec`, or a raw syscall. Its mediation boundary covers commands run through its MCP, Python, and Node.js surfaces; it does not extend to other tools registered with an agent. You declare which host files, internal URLs, and credentials the shell can reach. Network access follows the [default policy](/docs/user-guide/shell/configuration/index.md#network-access). The source is on [GitHub](https://github.com/strands-agents/shell).
...
Your code talks to the shell through one of three surfaces: an MCP server, the Python API, or the Node.js API. Every command, file read, and network request initiated inside Strands Shell flows through the Kernel, which is the single mediation boundary.
...
A fresh shell starts without host files or credentials and uses the [default network policy](/docs/user-guide/shell/configuration/index.md#network-access). You grant additional access explicitly through three mechanisms:
...
The [Configuration](/docs/user-guide/shell/configuration/index.md) guide covers each of these in depth, including the TOML format that lets you declare the whole policy in a file.

## What Strands Shell is not

Strands Shell is a mediation layer, not a hardened sandbox. The Kernel enforces its least-privilege policy inside the shell, and it runs in the same process as your code. It doesn’t protect against memory-safety exploits in the shell engine itself, timing side channels, or an attacker who already controls the host process.
...
-   [Quickstart](/docs/user-guide/shell/quickstart/index.md): install the shell and run your first sandboxed command through MCP, Python, or Node.js.
-   [Configuration](/docs/user-guide/shell/configuration/index.md): bind directories, inject credentials, set the network allowlist, and load it all from TOML.
-   [Commands](/docs/user-guide/shell/commands/index.md): the full command inventory, supported flags, and known gaps versus GNU coreutils.
-   [MCP Server](/docs/user-guide/shell/mcp-server/index.md): expose the shell to any MCP-compatible agent framework over stdio.
-   [Security Model](/docs/user-guide/shell/security/index.md): the Kernel boundary, the SSRF guard, credential handling, and how to layer OS isolation for adversarial workloads.