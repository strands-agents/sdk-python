...
The Bedrock Nova Sonic provider requires Python 3.12 or higher due to its experimental AWS SDK dependency.

Bedrock Nova Sonic is configured as an optional dependency in Strands Agents.
...
Nova Sonic requires AWS credentials for access. Under the hook, `BedrockNovaSonicModel` uses an experimental [Bedrock client](https://github.com/aws/aws-sdk-python/tree/develop/clients/aws-sdk-bedrock-runtime/src/aws_sdk_bedrock_runtime), which allows for credentials to be configured in the following ways:
