...
This is the foundational concept in the Strands Agents SDK, installed as `strands-agents` on PyPI and `@strands-agents/sdk` on npm. Everything else builds on top of it. The loop ships with the controls a production agent needs:
...
-   [Concurrent invocations](#concurrent-invocations) guard against overlapping calls on one agent instance
-   [Hooks](/docs/user-guide/concepts/agents/hooks/index.md) intercept any step, and [retry strategies](/docs/user-guide/concepts/agents/retry-strategies/index.md) recover from model and tool errors
...
The agent emits lifecycle events at key points: before and after each invocation, before and after each model call, and before and after each tool execution. These events enable observation, metrics collection, and behavior modification without changing the core loop logic. See [Hooks](/docs/user-guide/concepts/agents/hooks/index.md) for details on subscribing to these events.
...
You can also pass your own `threading.Event` into `__call__()`, `invoke_async()`, or `stream_async()` via the `cancel_signal` parameter. The agent observes both its internal signal and the external event, so `agent.cancel()` and the external signal can each trigger cancellation independently. The agent never sets or clears the caller’s event, and an event that is already set cancels the invocation at its first checkpoint. This is useful for declarative timeouts, sharing one signal across agents, or framework-driven cancellation on client disconnect. Two costs to know about: the first checkpoint sits inside model streaming, so an event that is set before the call still records the user turn and may issue (then abort) one model request; and the agent observes the event by polling (up to ~50 ms latency, compounding per `as_tool()` nesting level), so a set-then-cleared event inside one poll window is missed — clear the event only between invocations.
...
threading.Timer(30.0, cancel_signal.set).start()  # declarative timeout
...
The signal is also forwarded to the model provider so it can abort the in-flight request instead of letting the response run on. Support is provider-dependent: Bedrock closes the streaming response at the next chunk boundary, but cannot abort a request that has not returned headers yet or a non-streaming (`streaming=False`) call. Providers that ignore the signal still cancel at the SDK’s between-chunk checkpoint.

#### Cancellation within tool execution

The SDK automatically checks for cancellation before and between tool calls (see checkpoints above). Once a tool is running, cancellation is **cooperative**: only the tool itself can respond mid-execution. Tools can participate by polling `tool_context.cancel_signal.is_set()` between steps, or by forwarding `tool_context.cancel_signal` to APIs that accept one (e.g. `MCPClient.call_tool_async`). If a tool does neither, it runs to completion, and the agent resumes cancellation handling after the tool returns. An agent used as a tool via `as_tool()` receives the parent’s signal automatically, so cancelling the parent also cancels the delegated agent.
...
-   [Conversation Management](/docs/user-guide/concepts/agents/conversation-management/index.md) strategies that maintain coherent long-running interactions
-   [Hooks](/docs/user-guide/concepts/agents/hooks/index.md) for observing, modifying, and extending agent behavior
