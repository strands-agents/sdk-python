...
Snapshots are restored verbatim

A snapshot’s `messages` are loaded into the agent as-is, including any tool-call content they carry. Load snapshots only from a source you control. A snapshot influenced by an untrusted party is untrusted input: its content reaches the model, and in the Python SDK a trailing tool-call block runs a tool directly on the next invocation. See [Trusted Message History](/docs/user-guide/safety-security/trusted-message-history/index.md).
