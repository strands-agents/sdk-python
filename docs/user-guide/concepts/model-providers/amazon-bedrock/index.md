...
(( tab "Python" ))
The [`BedrockModel`](/docs/api/python/strands.models.bedrock) provider is used by default when creating a basic Agent, and uses the [Claude Sonnet 4](https://aws.amazon.com/blogs/aws/claude-opus-4-anthropics-most-powerful-model-for-coding-is-now-in-amazon-bedrock/) model by default. This basic example creates an agent using this default setup:
...
(( tab "Python" ))
Amazon Bedrock supports guardrails to help ensure model outputs meet your requirements. Strands allows you to configure guardrails with your [`BedrockModel`](/docs/api/python/strands.models.bedrock):
...
When you enable prompt caching, Amazon Bedrock creates a cache composed of **cache points**. These are markers that define the contiguous subsection of your prompt that you wish to cache. Cached content must remain unchanged between requests - any alteration invalidates the cache.
...
(( tab "Python" ))
Strands allows you to enable and configure reasoning capabilities with your [`BedrockModel`](/docs/api/python/strands.models.bedrock):
...
(( tab "TypeScript" ))
Strands allows you to enable and configure reasoning capabilities with your [`BedrockModel`](/docs/api/typescript/BedrockModel/index.md):
