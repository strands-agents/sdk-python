...
- **A merged change to `test-infra/` deploys itself**: `.github/workflows/test-infra-deploy.yml` deploys the stack to the team's test account on push to `main`, so the live IAM role can't drift from the code. A PR gets a read-only `cdk diff` posted to it, and can be deployed before merge only by approving the `test-infra-deploy-approval` environment — which also lets that PR's code run with deploy credentials, so read the code, not just the diff.
