# ABOUTME: Tests for experimental AgentConfig with to_agent() method
# ABOUTME: Validates configuration loading and Agent instance creation
...
class TestAgentConfig:
    ...
    def test_to_agent_calls_agent_constructor(self, mock_agent):
        ...
        mock_agent.assert_called_once_with(
            model="test-model",
            tools=[],  # ToolPool converts to empty list
            system_prompt="Test prompt"
        )
    
    def test_agent_config_has_tool_pool(self):
        """Test AgentConfig creates ToolPool and tracks configured tools."""
    ...
    def test_to_agent_with_empty_tool_pool(self, mock_agent):
        """Test that to_agent uses empty ToolPool by default."""
    ...
    def test_to_agent_with_tool_pool_parameter(self, mock_agent):
        """Test that to_agent uses configured ToolPool."""
    ...
    def test_agent_config_skip_missing_tools(self):
        ...
        # Should not raise error when flag is False and no ToolPool provided
    ...
    def test_agent_config_skip_missing_tools_with_selection(self):
        """Test that missing tools in ToolPool can be skipped with flag."""
        # Create custom ToolPool with one tool
        from strands.types.tools import AgentTool
    ...
    def test_agent_config_missing_tool_validation_with_flag_true(self):
        ...
        with pytest.raises(ValueError, match="Tool 'missing_tool' not found in ToolPool"):
            AgentConfig({
                "model": "test-model",
                "tools": ["missing_tool"]
            }, tool_pool=custom_pool, raise_exception_on_missing_tool=True)
