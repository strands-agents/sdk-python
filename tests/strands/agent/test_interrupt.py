...
@pytest.fixture
def interrupt():
    return Interrupt(id_="test_id", name="test_name", reason="test reason")
...
def test_interrupt_state_setdefault(interrupt):
    ...
    tru_interrupt = interrupt_state.setdefault("test_id", Interrupt(id_="dummy_id", name="dummy_name"))
    exp_interrupt = interrupt
    assert tru_interrupt == exp_interrupt
...
def test_interrupt_state_to_dict(interrupt):
    ...
    exp_data = {
        "interrupts": {"test_id": {"id_": "test_id", "name": "test_name", "reason": "test reason", "response": None}},
...
def test_interrupt_state_from_dict():
    ...
    exp_state = InterruptState(
        interrupts={"test_id": Interrupt(id_="test_id", name="test_name", reason="test reason")},
        context={"test": "context"},
        activated=True,
    )
