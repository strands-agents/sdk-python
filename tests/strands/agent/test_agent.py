...
def test_agent_cleanup(agent):
    ...
    with unittest.mock.patch("strands.agent.agent.run_async") as mock_run_async:
        ...
        called_func = mock_run_async.call_args[0][0]
        assert called_func == agent.cleanup_async


@pytest.mark.asyncio
async def test_agent_cleanup_async(agent):
...
def test_agent_cleanup_idempotent(agent):
...
def test_agent__del__emits_warning_for_automatic_cleanup():
    ...
    with unittest.mock.patch.object(agent, "cleanup") as mock_cleanup:
        with warnings.catch_warnings(record=True) as w:
            ...
            assert "Agent cleanup called via __del__" in str(w[0].message)
...
def test_agent__del__no_warning_when_no_tool_providers():
    ...
    with unittest.mock.patch.object(agent, "cleanup") as mock_cleanup:
        with warnings.catch_warnings(record=True) as w:
            ...
            # Verify no warning was emitted and cleanup wasn't called
            assert len(w) == 0
