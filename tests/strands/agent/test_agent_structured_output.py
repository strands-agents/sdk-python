...
class TestAgentStructuredOutputInvocation:
    ...
    def test_agent_call_with_structured_output_model(self, mock_event_loop, user_model, mock_model):
        """Test Agent.__call__ with structured_output_model parameter."""

        # Setup mock event loop
        async def mock_cycle(*args, **kwargs):
            # Check that structured_output_context was passed correctly
        ...
        result = agent("Extract user info", structured_output_model=user_model)

        # Verify result has structured output
        # The structured_output should be in the state for now
        assert result.state is not None
        assert "structured_output" in result.state
        assert isinstance(result.state["structured_output"], UserModel)
        assert result.state["structured_output"].name == "John"
    ...
    @patch("strands.agent.agent.event_loop_cycle")
    def test_agent_call_with_default_structured_output_model(self, mock_event_loop, product_model, mock_model):
        ...
        async def mock_cycle(*args, **kwargs):
            ...
            yield EventLoopStopEvent(
                ...
                {"role": "assistant", "content": [{"text": "Response"}]},
                {},
                {"structured_output": ProductModel(title="Widget", price=9.99)},
            )
        ...
        assert result.state is not None
        assert "structured_output" in result.state
        assert isinstance(result.state["structured_output"], ProductModel)
        assert result.state["structured_output"].title == "Widget"

    @patch("strands.agent.agent.event_loop_cycle")
    def test_agent_call_override_default_structured_output_model(
        self, mock_event_loop, user_model, product_model, mock_model
    ):
        ...
        async def mock_cycle(*args, **kwargs):
            ...
            yield EventLoopStopEvent(
                ...
                {"role": "assistant", "content": [{"text": "Response"}]},
                {},
                {"structured_output": UserModel(name="Jane", age=25, email="jane@example.com")},
            )
        ...
        assert "structured_output" in result.state
        assert isinstance(result.state["structured_output"], UserModel)
        assert result.state["structured_output"].name == "Jane"

    @pytest.mark.asyncio
    @patch("strands.agent.agent.event_loop_cycle")
    async def test_agent_invoke_async_with_structured_output(self, mock_event_loop, user_model, mock_model):
        ...
        async def mock_cycle(*args, **kwargs):
            ...
            yield EventLoopStopEvent(
                ...
                {"role": "assistant", "content": [{"text": "Response"}]},
                {},
                {"structured_output": UserModel(name="Alice", age=28, email="alice@example.com")},
            )
        ...
        assert "structured_output" in result.state
        assert isinstance(result.state["structured_output"], UserModel)
        assert result.state["structured_output"].name == "Alice"

    @pytest.mark.asyncio
    @patch("strands.agent.agent.event_loop_cycle")
    async def test_agent_stream_async_with_structured_output(self, mock_event_loop, product_model, mock_model):
        ...
        async def mock_cycle(*args, **kwargs):
            ...
            yield EventLoopStopEvent(
                ...
                {"role": "assistant", "content": [{"text": "Response"}]},
                {},
                {"structured_output": ProductModel(title="Gadget", price=19.99, description="Cool gadget")},
            )
        ...
        assert "structured_output" in result.state
        assert isinstance(result.state["structured_output"], ProductModel)
        assert result.state["structured_output"].title == "Gadget"
...
class TestAgentStructuredOutputContext:
    ...
    @patch("strands.agent.agent.event_loop_cycle")
    def test_structured_output_context_created_with_model(self, mock_event_loop, user_model, mock_model):
        """Test that StructuredOutputContext is created when structured_output_model is provided."""
        captured_context = None

        async def mock_cycle(*args, **kwargs):
            nonlocal captured_context
            captured_context = kwargs.get("structured_output_context")
            yield EventLoopStopEvent("end_turn", {"role": "assistant", "content": [{"text": "Response"}]}, {}, {})
        ...
        assert isinstance(captured_context, StructuredOutputContext)
        assert captured_context.structured_output_model == user_model
        assert captured_context.is_enabled is True

    @patch("strands.agent.agent.event_loop_cycle")
    def test_structured_output_context_none_without_model(self, mock_event_loop, mock_model):
        ...
        assert captured_context is not None
        assert isinstance(captured_context, StructuredOutputContext)
        assert captured_context.structured_output_model is None
        assert captured_context.is_enabled is False

    @patch("strands.tools.registry.ToolRegistry.register_dynamic_tool")
    @patch("strands.agent.agent.event_loop_cycle")
    def test_structured_output_tool_registered_dynamically(
        self, mock_event_loop, mock_register, user_model, mock_model
    ):
...
class TestAgentStructuredOutputEdgeCases:
    ...
    @patch("strands.agent.agent.event_loop_cycle")
    def test_agent_with_no_structured_output(self, mock_event_loop, mock_model):
        ...
        async def mock_cycle(*args, **kwargs):
            ...
            yield EventLoopStopEvent(
                "end_turn", {"role": "assistant", "content": [{"text": "Normal response"}]}, {}, {}
            )
        ...
        assert "structured_output" not in result.state
        assert result.message["content"][0]["text"] == "Normal response"

    def test_agent_multiple_structured_output_models(self, user_model, product_model):
        ...
        with patch("strands.agent.agent.event_loop_cycle") as mock_event_loop:
            ...
            async def mock_user_cycle(*args, **kwargs):
                ...
                yield EventLoopStopEvent(
                    ...
                    {"role": "assistant", "content": [{"text": "User response"}]},
                    {},
                    {"structured_output": UserModel(name="Bob", age=40, email="bob@example.com")},
                )

            mock_event_loop.side_effect = mock_user_cycle
            result1 = agent("Get user", structured_output_model=user_model)
            assert "structured_output" in result1.state
            assert isinstance(result1.state["structured_output"], UserModel)
        ...
        with patch("strands.agent.agent.event_loop_cycle") as mock_event_loop:
            ...
            async def mock_product_cycle(*args, **kwargs):
                ...
                yield EventLoopStopEvent(
                    ...
                    {"role": "assistant", "content": [{"text": "Product response"}]},
                    {},
                    {"structured_output": ProductModel(title="Item", price=5.99)},
                )

            mock_event_loop.side_effect = mock_product_cycle
            result2 = agent("Get product", structured_output_model=product_model)
            assert "structured_output" in result2.state
            assert isinstance(result2.state["structured_output"], ProductModel)
