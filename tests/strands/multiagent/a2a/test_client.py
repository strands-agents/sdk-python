...
class TestA2AClientRequestBuilding:
    ...
    def test_build_task_subscribe_request(self):
        """Test building a task/sendSubscribe request."""
        client = A2AClient(url="http://localhost:9000")
        request = client._build_task_subscribe_request("task-123", "session-456", "Hello")
...
class TestA2AClientResponseParsing:
    ...
    def test_parse_sse_line_valid(self):
        ...
        client = A2AClient(url="http://localhost:9000")
        line = 'data: {"result": {"status": {"message": {"parts": [{"kind": "text", "text": "Hello"}]}}}}'
        assert client._parse_sse_line(line) == "Hello"

    def test_parse_sse_line_non_data(self):
        """Test parsing non-data SSE line returns None."""
        client = A2AClient(url="http://localhost:9000")
        assert client._parse_sse_line("event: message") is None

    def test_parse_sse_line_empty_data(self):
        """Test parsing empty data SSE line returns None."""
        client = A2AClient(url="http://localhost:9000")
        assert client._parse_sse_line("data: ") is None
