...
def test_threading_instrumentation_is_on_by_default():
    """Constructing Tracer() MUST monkey-patch ThreadPoolExecutor.submit by default.

    Threading instrumentation is ENABLED by default: this preserves the
    historical strands>=1.35.0 behavior, so enabling opt-out levers is a
    non-breaking, additive change in a minor release. Users who do not want
    the global ``concurrent.futures.ThreadPoolExecutor.submit`` wrapper opt
    OUT via ``Tracer(instrument_threading=False)`` or
    ``OTEL_PYTHON_DISABLED_INSTRUMENTATIONS=threading`` (see the dedicated
    tests below). A future major (v2) may flip this default to off.
    """
    script = _WRAP_DETECT.format(construct="Tracer()")
    # Scrub ambient opt-out levers so a dev/CI shell that exported either of
    # them globally doesn't mask the default-on behavior under test.
    code, out = _run_in_subprocess(
        script,
        env={
            "STRANDS_INSTRUMENT_THREADING": None,
            "OTEL_PYTHON_DISABLED_INSTRUMENTATIONS": None,
        },
    )
    assert code == 0, f"subprocess failed: {out}"
    # Token match, not substring: ``"WRAPPED" in "UNWRAPPED"`` is True, so a
    # naive substring check can never distinguish the two outcomes. The detect
    # snippet prints exactly ``WRAPPED`` or ``UNWRAPPED`` on its own line.
    assert "WRAPPED" in out.split(), (
        "Tracer() did NOT wrap ThreadPoolExecutor.submit by default; "
        "threading instrumentation must be ON by default (non-breaking). "
        "Output:\n" + out
    )


def test_threading_instrumentation_env_var_explicit_off_disables():
    """STRANDS_INSTRUMENT_THREADING=false MUST disable the default-ON wrapper.

    This is the discriminating coverage for the new env opt-OUT branch
    introduced when the default flipped to ON. With default-ON, an explicit
    falsy env value (``0``/``false``/``no``) is the only env-level lever that
    changes behavior, so it must be proven to actually UNWRAP. We scrub the
    OTEL disable var so the opt-out under test is the *only* thing that can
    flip the result to UNWRAPPED (otherwise the OTEL var could mask it and
    make this test tautological again).
    """
    script = _WRAP_DETECT.format(construct="Tracer()")
    code, out = _run_in_subprocess(
        script,
        env={
            "STRANDS_INSTRUMENT_THREADING": "false",
            "OTEL_PYTHON_DISABLED_INSTRUMENTATIONS": None,
        },
    )
    assert code == 0, f"subprocess failed: {out}"
    assert "UNWRAPPED" in out.split(), (
        "Tracer() wrapped ThreadPoolExecutor.submit despite "
        "STRANDS_INSTRUMENT_THREADING=false; the explicit env opt-OUT branch "
        "must disable the default-ON wrapper. Output:\n" + out
    )
...
def test_threading_instrumentation_opt_in_via_env_var():
    ...
    assert code == 0, f"subprocess failed: {out}"
    assert "WRAPPED" in out.split(), "Explicit opt-in did not wrap ThreadPoolExecutor.submit. Output:\n" + out
...
def test_threading_instrumentation_opt_in_via_kwarg():
    ...
    assert code == 0, f"subprocess failed: {out}"
    assert "WRAPPED" in out.split(), (
        "Tracer(instrument_threading=True) did not wrap ThreadPoolExecutor.submit. Output:\n" + out
    )
...
def test_threading_instrumentation_swallows_instrumentor_failures():
    ...
    """
    # Scrub ambient opt-out levers so a dev/CI shell that exported either of
    # them globally doesn't short-circuit _maybe_instrument_threading before
    # the patched instrument() (_boom) is reached — otherwise the swallow path
    # is never exercised and this test passes vacuously.
    code, out = _run_in_subprocess(
        script,
        env={
            "OTEL_PYTHON_DISABLED_INSTRUMENTATIONS": None,
...
def test_threading_instrumentation_failure_logged_at_error_when_user_requested():
    ...
    # Scrub ambient opt-out levers so a dev/CI shell that exported either of
    # them globally doesn't short-circuit _maybe_instrument_threading before
    # the patched instrument() (_boom) is reached — otherwise no failure is
    # raised, no ERROR is logged, and this test fails spuriously.
    code, out = _run_in_subprocess(
        script,
        env={
            "OTEL_PYTHON_DISABLED_INSTRUMENTATIONS": None,
...
def test_threading_instrumentation_env_var_off_word_disables():
    """STRANDS_INSTRUMENT_THREADING=off MUST disable the default-ON wrapper.

    ``off`` is a natural opt-out token a user would reasonably reach for
    alongside ``false``/``0``/``no``. Because instrumentation is ON by
    default, the opt-OUT direction is the load-bearing one: if ``off`` is not
    recognized it silently falls through to default-ENABLED and the user's
    intended opt-out is ignored. We scrub the OTEL disable var so the
    ``off`` opt-out under test is the only thing that can flip the result to
    UNWRAPPED.
    """
    script = _WRAP_DETECT.format(construct="Tracer()")
    code, out = _run_in_subprocess(
        script,
        env={
            "STRANDS_INSTRUMENT_THREADING": "off",
            "OTEL_PYTHON_DISABLED_INSTRUMENTATIONS": None,
        },
    )
    assert code == 0, f"subprocess failed: {out}"
    assert "UNWRAPPED" in out.split(), (
        "Tracer() wrapped ThreadPoolExecutor.submit despite "
        "STRANDS_INSTRUMENT_THREADING=off; the 'off' token must be honored as "
        "an explicit env opt-OUT. Output:\n" + out
    )


def test_threading_instrumentation_unrecognized_value_warns_and_defaults():
    """An unrecognized non-empty value warns and falls through to default-ON.

    A typo like ``flase`` matches neither the on nor the off token set. Rather
    than silently swallowing the (likely opt-out) intent, the resolver must log
    a WARNING naming the unrecognized value, then apply the default (ON). We
    capture the subprocess's logging to stderr and assert both the WRAPPED
    default behavior and the presence of the warning text.
    """
    script = """
        import logging
        import sys
    ...
    code, out = _run_in_subprocess(
        script,
        env={
            "STRANDS_INSTRUMENT_THREADING": "flase",
            "OTEL_PYTHON_DISABLED_INSTRUMENTATIONS": None,
        },
    )
    assert code == 0, f"subprocess failed: {out}"
    assert "WRAPPED" in out.split(), (
        "Tracer() did NOT wrap ThreadPoolExecutor.submit for an unrecognized "
        "STRANDS_INSTRUMENT_THREADING value; an unparseable value must fall "
        "through to the default (ON). Output:\n" + out
    )
    assert "STRANDS_INSTRUMENT_THREADING" in out, (
        "No warning naming STRANDS_INSTRUMENT_THREADING was logged for an "
        "unrecognized value; the opt-out intent must not be silently swallowed. "
        "Output:\n" + out
    )
    assert "flase" in out, (
        "Warning did not echo the unrecognized value 'flase'; operators can't "
        "tell what tripped the default. Output:\n" + out
    )
