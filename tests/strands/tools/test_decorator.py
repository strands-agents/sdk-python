...
@pytest.mark.asyncio
async def test_strands_context_excluded_from_schema(alist):
    """Test that strands_context parameter is excluded from the tool's input schema."""
    
    @strands.tool
    def context_tool(message: str, count: int, strands_context: StrandsContext) -> str:
        """Tool with both regular and context parameters."""
        return f"Message: {message}, Count: {count}"
    
    # Check that the tool spec excludes strands_context from the schema
    tool_spec = context_tool.tool_spec
    schema_properties = tool_spec["inputSchema"]["json"].get("properties", {})
    
    # Regular parameters should be included
    assert "message" in schema_properties
    assert "count" in schema_properties
    
    # StrandsContext parameter should be excluded
    assert "strands_context" not in schema_properties


@pytest.mark.asyncio
async def test_strands_context_optional_parameter(alist):
    """Test that tools work correctly whether they use StrandsContext or not."""
    
    @strands.tool
    def tool_without_context(message: str) -> dict:
        """Tool that doesn't use StrandsContext."""
        return {
            "status": "success",
            "content": [{"text": f"Processed: {message}"}]
        }
    
    @strands.tool
    def tool_with_context(message: str, strands_context: StrandsContext) -> dict:
        """Tool that uses StrandsContext."""
        tool_use_id = strands_context["tool_use"]["toolUseId"]
        return {
            "status": "success", 
            "content": [{"text": f"Processed: {message} (ID: {tool_use_id})"}]
        }
    
    # Test tool without context
    tool_use_1 = {"toolUseId": "test-1", "input": {"message": "hello"}}
    stream_1 = tool_without_context.stream(tool_use_1, {})
    result_1 = (await alist(stream_1))[-1]
    
    assert result_1["status"] == "success"
    assert "Processed: hello" in result_1["content"][0]["text"]
    
    # Test tool with context
    tool_use_2 = {"toolUseId": "test-2", "input": {"message": "world"}}
    stream_2 = tool_with_context.stream(tool_use_2, {})
    result_2 = (await alist(stream_2))[-1]
    
    assert result_2["status"] == "success"
    assert "Processed: world (ID: test-2)" in result_2["content"][0]["text"]


@pytest.mark.asyncio
async def test_strands_context_with_complex_invocation_state(alist):
    """Test StrandsContext with complex invocation state data."""
    
    @strands.tool
    def storage_tool(data: str, strands_context: StrandsContext) -> dict:
        """Tool that accesses complex invocation state."""
        tool_use_id = strands_context["tool_use"]["toolUseId"]
        global_storage = strands_context["invocation_state"].get("global_storage", {})
        session_id = strands_context["invocation_state"].get("session_id", "unknown")
        
        # Simulate storing data
        global_storage[tool_use_id] = {
            "data": data,
        ...
        return {
            "status": "success",
            "content": [{"text": f"Stored '{data}' with ID {tool_use_id} in session {session_id}"}]
        }
    
    # Create complex invocation state
    global_storage = {}
    invocation_state = {
        "agent": "test-agent",
    ...
    tool_use = {
        "toolUseId": "storage-123",
        "name": "storage_tool",
        "input": {"data": "important info"}
    }
    
    stream = storage_tool.stream(tool_use, invocation_state)
    result = (await alist(stream))[-1]
    
    assert result["status"] == "success"
    assert "Stored 'important info' with ID storage-123 in session session-456" in result["content"][0]["text"]
    
    # Verify the global storage was modified
    assert "storage-123" in global_storage
    assert global_storage["storage-123"]["data"] == "important info"
    assert global_storage["storage-123"]["session_id"] == "session-456"


@pytest.mark.asyncio
async def test_strands_context_direct_function_call(alist):
    """Test that tools with StrandsContext can still be called directly (without context)."""
    
    @strands.tool
    def flexible_tool(message: str, strands_context: StrandsContext = None) -> str:
        """Tool that can work with or without context."""
        if strands_context:
            tool_use_id = strands_context["tool_use"]["toolUseId"]
            return f"Tool use {tool_use_id}: {message}"
        else:
            return f"Direct call: {message}"
    
    # Test direct function call (should work with default None)
    result = flexible_tool("hello direct")
    assert result == "Direct call: hello direct"
    
    # Test tool use call (context should be injected)
    tool_use = {
        "toolUseId": "flexible-123",
        "name": "flexible_tool",
        "input": {"message": "hello tool use"}
    }
    
    stream = flexible_tool.stream(tool_use, {})
    tool_result = (await alist(stream))[-1]
    
    assert tool_result["status"] == "success"
    assert "Tool use flexible-123: hello tool use" in tool_result["content"][0]["text"]


@pytest.mark.asyncio
async def test_strands_context_async_tool(alist):
    """Test StrandsContext injection with async tools."""
    
    @strands.tool
    async def async_context_tool(message: str, strands_context: StrandsContext) -> dict:
        """Async tool that uses StrandsContext."""
        tool_use_id = strands_context["tool_use"]["toolUseId"]
        # Simulate async work
        import asyncio
        ...
        return {
            "status": "success",
            "content": [{"text": f"Async tool {tool_use_id} processed: {message}"}]
        }
    
    tool_use = {
        "toolUseId": "async-123",
        "name": "async_context_tool",
        "input": {"message": "async hello"}
    }
    
    stream = async_context_tool.stream(tool_use, {})
    result = (await alist(stream))[-1]
    
    assert result["status"] == "success"
    assert "Async tool async-123 processed: async hello" in result["content"][0]["text"]
