...
class TestDelegationIntegration:
    ...
    async def test_end_to_end_delegation_flow(self):
        ...
        delegation_msg_found = any(
            "Delegated from Orchestrator" in str(msg.get("content", []))
    ...
    async def test_delegation_exception_raised_in_tool(self):
        ...
        orchestrator = Agent(
            name="Orch",
            model=MockedModelProvider([
                {"role": "assistant", "content": [{"text": "Orch response"}]}
            ]),
            sub_agents=[sub_agent]
        )
    ...
    async def test_state_transfer_deep_copy(self):
        """Test state is deep copied during delegation."""
        sub_agent = Agent(
            name="Sub",
            model=MockedModelProvider([
        ...
        orchestrator.state = {"nested": {"data": "original"}}
        orchestrator.messages = [{"role": "user", "content": [{"text": "Test"}]}]
        
        # Execute delegation
        await orchestrator.invoke_async()
        
        # Verify state was transferred (convert AgentState to dict for comparison)
        # Agent.state can be AgentState wrapper or dict
        sub_state = dict(sub_agent.state) if hasattr(sub_agent.state, '__iter__') else sub_agent.state
        orch_state = dict(orchestrator.state) if hasattr(orchestrator.state, '__iter__') else orchestrator.state
        assert sub_state == orch_state
        # Note: Deep copy ensures modification of one doesn't affect the other
    ...
    async def test_message_filtering_integration(self):
        ...
        # Set orchestrator messages with noise
        orchestrator.messages = [
            {"role": "system", "content": [{"text": "System prompt"}]},
            {"role": "user", "content": [{"text": "User query"}]},
            {"role": "assistant", "content": [
                {"toolUse": {"name": "internal_tool", "toolUseId": "t1", "input": {}}},  # Should be filtered
            ]},
            {"role": "assistant", "content": [{"text": "Response"}]},  # Should be kept
        ]
        
        # Execute delegation
        await orchestrator.invoke_async()
        
        # Verify sub-agent received filtered messages
        ...
        assert any(
            msg.get("role") == "user" and "Delegated from Orch" in str(msg.get("content"))
            for msg in sub_agent.messages
        )
    ...
    async def test_target_agent_not_found_error(self):
        ...
        fake_exception = AgentDelegationException(
    ...
    async def test_delegation_context_always_added(self):
        ...
        delegation_msg_found = any(
            "Delegated from Orch: Important task" in str(msg.get("content", []))
            for msg in sub_agent.messages
        )
        assert delegation_msg_found
    ...
    async def test_additional_context_transfer(self):
        ...
        context_msg_found = any(
            "Additional context:" in str(msg.get("content", []))
            and "user_id" in str(msg.get("content", []))
            for msg in sub_agent.messages
        )
    ...
    async def test_streaming_proxy_integration(self):
        ...
        sub_agent = Agent(name="StreamingSub", model=MockedModelProvider([
            {"role": "assistant", "content": [{"text": "Streaming response"}]}
        ]))
    ...
    async def test_session_persistence_integration(self):
        ...
        sub_agent = Agent(
            name="SessionSub",
            model=MockedModelProvider([
                {"role": "assistant", "content": [{"text": "Session response"}]}
            ])
        )
        ...
        delegation_msg_found = any(
            "Delegated from Orch" in str(msg.get("content", []))
            for msg in sub_agent.messages
        )
        assert delegation_msg_found
    ...
    async def test_nested_delegation_chain_integration(self):
        ...
        level2_agent = Agent(
            name="Level2",
            model=MockedModelProvider([
                {"role": "assistant", "content": [{"text": "Level2 response"}]}
            ])
        )
        ...
        level1_delegation = any(
            "Delegated from Orch" in str(msg.get("content", []))
            for msg in level1_agent.messages
        )
        level2_delegation = any(
            "Delegated from Level1" in str(msg.get("content", []))
            for msg in level2_agent.messages
        )
