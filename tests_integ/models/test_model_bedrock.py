...
def test_prompt_caching_cache_tools_ttl():
    """Test that cache_config.ttl propagates into the auto-injected toolConfig cache point.

    Verifies that BedrockModel(cache_tools="default", cache_config=CacheConfig(ttl="5m")) produces
    a Bedrock request with cachePoint.ttl on the toolConfig checkpoint, and that the call completes
    without a ValidationException on the TTL field.
