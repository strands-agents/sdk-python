...
def _resolve_middleware_interrupt(
    ...
    Args:
        interrupts: The prior-response lookup — the agent's live ``interrupts`` dict for the tool
            stage, or a snapshot taken before the pass for the agent-stream stage (whose reads
            must survive a mid-pass ``deactivate()`` from a tool cycle in the same pass).
...
class AgentStreamContext:
    ...
    # lifetime spans the whole pass: a tool cycle within the same pass calls the event loop's
    # deactivate(), which clears the live interrupts — reading a snapshot keeps a gate's re-read
    # after next_fn stable. Required (the run loop is the sole constructor and always supplies
    # it); excluded from repr to avoid dumping unrelated interrupt bookkeeping.
    ...
    def interrupt(self, name: str, *, reason: Any = None, response: Any = None) -> MiddlewareInterruptResult:
        ...
        Args:
            name: User-defined name for the interrupt. The interrupt id is scoped to the
                agent-stream namespace (``v1:middleware_agent_stream:<uuid5(name)>``), so the
                name must be unique across all agent-stream middleware in a single invocation
                pass — two middleware using the same name collide and share one response.
            reason: Optional reason for the interrupt (surfaced to the user).
            response: Optional preemptive response — when set, no interrupt is raised.
    ...
    def _interrupt_id(self, name: str) -> str:
        ...
        """
        return f"v1:middleware_agent_stream:{uuid.uuid5(uuid.NAMESPACE_OID, name)}"
