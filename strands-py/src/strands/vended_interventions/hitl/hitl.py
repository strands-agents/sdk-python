...
AskFunction = Callable[[str], Any | Awaitable[Any]]
"""Callable that presents a prompt to a human and returns their response.

May be a plain function or a coroutine function. The returned value (awaited if
necessary) is passed to the configured evaluate functions.
"""
...
class HumanInTheLoop(InterventionHandler):
    ...
    def __init__(
        ...
        *,
        ...
        evaluate_trust: Callable[[Any], bool] | None = None,
        ...
        Args:
            ...
            ask: Controls how the human's response is collected. Omitted (default):
                ...
                human responds. Custom callable: your own (optionally async) prompt
                logic (Slack, web UI, etc.). Agent blocks inline.
        """
    ...
    async def before_tool_call(  # type: ignore[override]
        ...
        if self._ask is None:
            ...
            def evaluate(response: Any) -> bool:
                if not is_negated and self._enable_trust and self._evaluate_trust(response):
                    ...
                    return True
                return self._evaluate(response) if self._evaluate is not None else default_evaluate(response)
        ...
        return Confirm(
            prompt=prompt,
            response=response,
            evaluate=self._evaluate if self._evaluate is not None else default_evaluate,
        )
