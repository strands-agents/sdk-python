...
class _DirectToolCall:
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        # Intentional: when record_direct_tool_call=False, the lock is skipped so that
        # direct tool calls can be made during an active agent invocation.
    ...
    def stream(self, **kwargs: Any) -> Iterator[Any]:
        ...
        Note:
            Early consumer break (e.g., ``break`` in a ``for`` loop) does not cancel
            the underlying tool execution. The tool will run to completion in the
            background thread before resources are released.
