...
class JsonMemoryStore(MemoryStore):
    ...
    The on-disk format is shared with the TypeScript SDK's ``JsonMemoryStore``: records use the same
    ...
    def __init__(self, **store_config: Unpack[JsonMemoryStoreConfig]) -> None:
        """Initialize the store.

        Args:
            **store_config: See :class:`JsonMemoryStoreConfig`.
        ...
        if not self.name.strip():
            raise ValueError("JsonMemoryStore: name must not be empty.")
        ...
        if max_search_results is not None and max_search_results < 1:
            raise ValueError("JsonMemoryStore: max_search_results must be at least 1.")
        ...
        if path is not None and not path.strip():
            raise ValueError("JsonMemoryStore: path must not be empty.")
    ...
    async def add(self, content: str, metadata: Metadata | None = None) -> JsonMemoryAddResult:
        ...
        if not self.writable:
            raise ValueError("JsonMemoryStore: store is not writable. Set writable=True in config to enable add().")
        if not content.strip():
            raise ValueError("JsonMemoryStore: content must not be empty.")
        ...
        with self._lock:
            ...
            for record in records:
                if record["content"].strip() == normalized_content:
                    return JsonMemoryAddResult(id=record["id"])
            ...
            return JsonMemoryAddResult(id=new_record["id"])
    ...
    def _load(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise ValueError(f"JsonMemoryStore: invalid JSON in {self._path}: {error}") from error
        except OSError as error:
            raise OSError(f"JsonMemoryStore: failed to read {self._path}: {error}") from error

        if not isinstance(parsed_file, list):
            raise ValueError(f"JsonMemoryStore: invalid backing file {self._path}: expected a JSON array of records")
        for record in parsed_file:
            if (
                ...
                raise ValueError(
                    f"JsonMemoryStore: invalid backing file {self._path}: "
    ...
    def _flush(self, records: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            raise OSError(f"JsonMemoryStore: failed to write {self._path}: {error}") from error
