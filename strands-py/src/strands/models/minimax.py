...
from .openai import Client, OpenAIModel
...
class MinimaxModel(OpenAIModel):
    ...
    MiniMax provides large language models including MiniMax-M3, accessible through an
    ...
    Example usage::
        ...
        model = MinimaxModel(model_id="MiniMax-M3")
        agent = Agent(model=model)
        response = agent("Tell me about AI")

    Attributes:
        client: The underlying OpenAI-compatible async client for the MiniMax API.
    """

    class MinimaxConfig(BaseModelConfig, total=False):
        """Configuration options for MiniMax models.

        Attributes:
            model_id: Model ID (e.g., "MiniMax-M3").
        ...
        **model_config: Unpack[MinimaxConfig],
        ...
        Args:
            ...
            client_args: Arguments for the OpenAI client.
                Defaults to using the MiniMax API base URL and the API key from the environment.
            **model_config: Configuration options for the MiniMax model.

        Raises:
            ValueError: If both ``client`` and ``client_args`` are provided.
        """
        # Apply MiniMax defaults only when the caller relies on auto-configuration.
        if client is None:
    ...
    def format_request(
        ...
        **kwargs: Any,
        ...
        Extends the OpenAI request formatting to drop an empty ``tools`` list, which the
        MiniMax API rejects.
        ...
        # MiniMax does not accept an empty tools list.
    ...
    def _clean_response_content(content: str) -> str:
        ...
        This strips those wrappers and returns only the meaningful content.
    ...
    async def structured_output(
        ...
        Uses a regular chat completion with ``response_format`` set to ``json_object`` instead of
        the beta parse API, since MiniMax models may emit ``<think>`` tags that interfere with the
        beta parser.
        ...
        # Add a schema hint as a user message so the model knows the expected format
        # (MiniMax only allows system messages at the beginning of the conversation).
        schema = json.dumps(output_model.model_json_schema())
        schema_hint = {
            "role": "user",
            "content": f"Respond with a JSON object matching this schema: {schema}",
        }
