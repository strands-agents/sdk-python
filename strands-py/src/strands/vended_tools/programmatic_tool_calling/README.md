# programmatic_tool_calling: recommendation to not vend

This directory intentionally contains no implementation. The recommendation
that programmatic_tool_calling should not be vended in its currently spec'd
form is recorded in
...
`strands-py/src/strands/agent/agent.py`, where `Agent.tool` returns a
`_ToolCaller` for `agent.tool.<name>(...)`-style invocation). That is a
