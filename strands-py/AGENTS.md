...
- **Cancellation is cooperative.** Tools receive the execution-scoped signal through
  `ToolContext.cancel_signal`, and tool middleware through
  `ExecuteToolContext.cancel_signal`; foreground and direct tool execution use the
  current agent cancellation event. Long-running integrations must propagate or observe
  the context signal rather than blocking the event loop. *(The TypeScript SDK uses
  `AbortSignal` instead — see its AGENTS.md.)*
