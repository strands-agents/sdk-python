...
class TestDropStrategy:
    ...
    async def test_replaces_tool_result_with_marker(self, mock_agent):
        ...
        result_content = messages[1]["content"][0]["toolResult"]["content"][0]["text"]
        assert result_content == DROPPED_MARKER
...
class TestMessageLevelDrop:
    """Tests for message-level drop with utilization threshold."""
...
class TestEmergencyTruncateStrategy:
    ...
    @pytest.mark.asyncio
    async def test_replace_block_returns_none(self, mock_agent):
        strategy = EmergencyTruncateStrategy()
        block = ContentBlock(text="anything")
        message = Message(role="user", content=[block])
        tru_result = await strategy._replace_block(block, 100, message, mock_agent)
...
class TestToolMatchesTarget:
    ...
    def test_returns_false_when_tool_name_not_in_map(self):
        ...
        tru_result = tool_matches_target(block, ["tool::bash"], {}, {"bash"}, None)
        assert tru_result is False

    def test_include_filter_matches_tool_name(self):
        block = self._make_tool_result_block(tool_use_id="tu-1")
        tool_name_map = {"tu-1": "bash"}
        tru_result = tool_matches_target(block, ["tool::bash"], tool_name_map, {"bash"}, None)
        assert tru_result is True

    def test_include_filter_rejects_non_matching_tool_name(self):
        block = self._make_tool_result_block(tool_use_id="tu-1")
        tool_name_map = {"tu-1": "python"}
        tru_result = tool_matches_target(block, ["tool::bash"], tool_name_map, {"bash"}, None)
        assert tru_result is False
    ...
    def test_exclude_filter_rejects_matching_tool_name(self):
        block = self._make_tool_result_block(tool_use_id="tu-1")
        tool_name_map = {"tu-1": "bash"}
        tru_result = tool_matches_target(block, ["!tool::bash"], tool_name_map, None, {"bash"})
        assert tru_result is False
    ...
    def test_returns_false_with_no_filters_and_list_target(self):
        block = self._make_tool_result_block(tool_use_id="tu-1")
        tool_name_map = {"tu-1": "bash"}
        tru_result = tool_matches_target(block, [], tool_name_map, None, None)
        assert tru_result is False
...
class TestTargetMatchesMessage:
    ...
    def test_assistant_text_target_matches_assistant_with_text(self):
        ...
        tru_result = target_matches_message("assistant_text", message)
        assert tru_result is True
    ...
    def test_user_text_target_matches_user_with_text(self):
        message = Message(role="user", content=[ContentBlock(text="hello")])
        tru_result = target_matches_message("user_text", message)
        assert tru_result is True
    ...
    def test_none_target_matches_any(self):
        message = Message(role="user", content=[ContentBlock(text="hello")])
        tru_result = target_matches_message(None, message)
        assert tru_result is True

    def test_wildcard_target_matches_any(self):
        message = Message(role="assistant", content=[ContentBlock(text="hello")])
        tru_result = target_matches_message("*", message)
        assert tru_result is True

    def test_unknown_string_target_returns_false(self):
        message = Message(role="user", content=[ContentBlock(text="hello")])
        tru_result = target_matches_message("tool_results", message)
        assert tru_result is False
...
class TestMessageMatchesTarget:
    ...
    def test_falls_through_to_tool_result_matching(self):
        ...
        tool_name_map = {"tu-1": "bash"}
        tru_result = message_matches_target(user_msg, "tool_results", tool_name_map, None, None)
        assert tru_result is True

    def test_target_none_guard_returns_false_for_non_text_match(self):
        """When target is None and target_matches_message returns True, it short-circuits.

        But when target is not None and target_matches_message returns False,
        then target=None guard is not hit. Test the None guard by passing a message
        that does NOT match text-level but has no tool results and target is None.
        """
        assistant_msg = Message(role="assistant", content=[ContentBlock(text="hello")])
        tru_result = message_matches_target(assistant_msg, "tool_results", {}, None, None)
        assert tru_result is False
    ...
    def test_returns_false_when_no_tool_results_match(self):
        ...
        user_msg = Message(role="user", content=[tool_result_block])
        tru_result = message_matches_target(user_msg, "tool_results", {}, None, None)
        assert tru_result is False
...
class TestGetOldestMatches:
    ...
    def test_returns_oldest_excluding_recent(self):
        ...
        assert tru_result[1] is messages[1]
    ...
    def test_returns_empty_when_count_exceeds_matches(self):
        ...
        tru_result = get_oldest_matches(messages, "*", 5, {}, None, None)
        assert tru_result == []
    ...
    def test_returns_empty_when_count_equals_matches(self):
        ...
        tru_result = get_oldest_matches(messages, "*", 1, {}, None, None)
        assert tru_result == []


class TestCollectRemovableWithPairExtended:
    """Extended tests for collect_removable_with_pair — tool-use/tool-result pair collection."""
    ...
    def test_head_pin_protection_prevents_pair_removal(self):
        """When the tool-use message IS the head pin (index 0), removing its result is blocked."""
        assistant_msg = Message(
            role="assistant",
            content=[ContentBlock(toolUse=ToolUse(toolUseId="tu-1", name="bash", input={}))],
        )
...
class TestSpliceWithPairsExtended:
    """Extended tests for splice_with_pairs — removal with pair safety."""
...
class TestResolveToolFilterExtended:
    """Extended tests for resolve_tool_filter — include/exclude/mixed."""
...
class TestEagerHookRegistration:
    """Tests for BaseOffloadStrategy.init — eager hook registration and firing."""
    ...
    def test_registers_eager_hook_for_per_block_strategy(self):
        ...
        strategy.init(agent)
        tru_callbacks = agent.hooks._registered_callbacks.get(MessageAddedEvent, [])
        assert len(tru_callbacks) == 1
...
class TestApplyPerMessage:
    """Tests for _apply_per_message — marker insertion and repair_alternation."""

    @pytest.mark.asyncio
    async def test_inserts_marker_and_repairs_alternation(self, mock_agent):
        strategy = Offload.drop("*").when(utilization=0.8)
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            Message(role="assistant", content=[ContentBlock(text="old1")]),
        ...
        mock_agent.messages = messages
        context = ContextState(messages=messages, agent=mock_agent, utilization=0.9)
        tru_acted = await strategy.apply(context)
        assert tru_acted is True
        for idx in range(len(messages) - 1):
            assert messages[idx]["role"] != messages[idx + 1]["role"]
    ...
    async def test_returns_false_when_splice_removes_nothing(self, mock_agent):
        """If splice_with_pairs cannot remove anything (e.g. head pin only), returns False."""
        strategy = Offload.drop("*").when(utilization=0.8)
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            Message(role="assistant", content=[ContentBlock(text="a")]),
...
class TestBlockMatchesTarget:
    ...
    def test_image_block_matches_wildcard(self):
        ...
        message = Message(role="user", content=[block])
        tru_result = strategy._block_matches_target(block, message, {})
        assert tru_result is True
...
class TestTransformBlocks:
    ...
    async def test_offloads_image_block(self, mock_agent):
        ...
        tru_acted = await strategy._transform_blocks(message, messages, {}, mock_agent)
        assert tru_acted is True
...
class TestGetEligibleMessages:
    ...
    async def test_preserve_recent_excludes_recent_matches(self, mock_agent):
        ...
        mock_agent.messages = messages
        context = ContextState(messages=messages, agent=mock_agent, utilization=0.9)
        tru_eligible = await strategy._get_eligible_messages(context)
        assert len(tru_eligible) <= 1
...
class TestTruncateStrategyPerMessage:
    """Tests for TruncateStrategy._apply_per_message — message-level truncation with head/tail preview."""
    ...
    @pytest.mark.asyncio
    async def test_returns_false_when_no_eligible_messages(self, mock_agent):
        strategy = Offload.truncate("assistant_text").when(utilization=0.8)
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            Message(role="user", content=[ContentBlock(text="only user messages")]),
        ]
    ...
    @pytest.mark.asyncio
    async def test_head_preview_keeps_head_removes_tail(self, mock_agent):
        strategy = Offload.truncate("*", {"preview": "head"}).when(utilization=0.8)
        head_msg = Message(role="assistant", content=[ContentBlock(text="head_keep")])
        tail_msg = Message(role="assistant", content=[ContentBlock(text="tail_msg")])
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            head_msg,
            Message(role="user", content=[ContentBlock(text="mid1")]),
            ...
            Message(role="user", content=[ContentBlock(text="mid5")]),
            tail_msg,
            Message(role="user", content=[ContentBlock(text="last")]),
        ]
        mock_agent.messages = messages
    ...
    @pytest.mark.asyncio
    async def test_tail_preview_keeps_tail_removes_head(self, mock_agent):
        strategy = Offload.truncate("*", {"preview": "tail"}).when(utilization=0.8)
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            Message(role="assistant", content=[ContentBlock(text="old1")]),
            Message(role="user", content=[ContentBlock(text="old2")]),
            Message(role="assistant", content=[ContentBlock(text="old3")]),
            Message(role="user", content=[ContentBlock(text="old4")]),
            Message(role="assistant", content=[ContentBlock(text="recent1")]),
            Message(role="user", content=[ContentBlock(text="recent2")]),
            Message(role="assistant", content=[ContentBlock(text="recent3")]),
            Message(role="user", content=[ContentBlock(text="recent4")]),
        ]
        mock_agent.messages = messages
        original_count = len(messages)
        context = ContextState(messages=messages, agent=mock_agent, utilization=0.9)
    ...
    @pytest.mark.asyncio
    async def test_head_tail_preview_keeps_both_ends(self, mock_agent):
        strategy = Offload.truncate("*", {"preview": "head_tail"}).when(utilization=0.8)
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            Message(role="assistant", content=[ContentBlock(text="head1")]),
            Message(role="user", content=[ContentBlock(text="mid1")]),
            Message(role="assistant", content=[ContentBlock(text="mid2")]),
            Message(role="user", content=[ContentBlock(text="mid3")]),
            Message(role="assistant", content=[ContentBlock(text="mid4")]),
    ...
    @pytest.mark.asyncio
    async def test_returns_false_when_splice_removes_nothing(self, mock_agent):
        """When splice_with_pairs cannot remove any middle messages, returns False."""
        strategy = Offload.truncate("*").when(utilization=0.8)
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            Message(role="assistant", content=[ContentBlock(text="a1")]),
        ...
        context = ContextState(messages=messages, agent=mock_agent, utilization=0.9)
        with unittest.mock.patch(
            "strands._context_manager.strategies.offload.truncate.splice_with_pairs",
            return_value=(0, 1),
        ):
            tru_acted = await strategy.apply(context)
        assert tru_acted is False
    ...
    @pytest.mark.asyncio
    async def test_preserve_recent_with_utilization(self, mock_agent):
    ...
    @pytest.mark.asyncio
    async def test_skips_when_below_utilization_threshold(self, mock_agent):
        strategy = Offload.truncate("*").when(utilization=0.8)
        messages: Messages = [
            Message(role="user", content=[ContentBlock(text="pin")]),
            Message(role="assistant", content=[ContentBlock(text="a1")]),
