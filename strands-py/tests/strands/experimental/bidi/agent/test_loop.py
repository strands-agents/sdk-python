...
@pytest.mark.asyncio
async def test_stop_closes_model_before_cancelling_reader(loop, agent):
    """Close the stream before cancelling its pending CRT response future."""
    order = []
    ...
    async def receive():
        ...
        try:
            await closed.wait()
            order.append("reader")
            yield BidiTextInputEvent(text="closed")
    ...
    # Guards normal shutdown against resolving a cancelled CRT future (aws/aws-sdk-python#13).
    assert order == ["model", "reader"]
